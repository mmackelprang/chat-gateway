...
job 3112, its **first real exercise**), MCP is enabled and **has delivered a real
...
future bonus — it ran, job 3112, SUCCESS. **The dated entries carrying them are
...
main -- src/` is empty. Previous entry follows.)
...
>    **CG-85** is decided.
>
> ✅ **The NAS memory half's numbers ARE in, and they are a clean result: 25.2 h,
> 152 samples, FLAT — `peak_bytes` unchanged, threads and fds constant, no leak.**
> They live in `docs/deploy/nas.md` §11 and CG-82; **not restated here.** Report it
> as *flat over 25 h*, never as the 72 h run that was designed.
...
to be a different verb.** The rebuild ran (`app.redeploy` job **3112**, SUCCESS,
its **first real exercise**), `?strict=1` is live, and the dashboard row now
...
deviation 6 records as documented-but-untested. ✅ **Claimed 2026-08-11: job 3112,
SUCCESS, against a locally built image on a box with no registry. It works**, and
that deviation no longer says *untested*.
...
`GATEWAY_ENABLE_MCP` was set in the compose via `app.update` (job **3180**).
