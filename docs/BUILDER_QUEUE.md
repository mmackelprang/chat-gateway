...
⚠ **CORRECTION BANNER — 2026-08-10 (Coordinator), EXTENDED 2026-08-11 (Builder).
Read this before the ENTIRE dated chain below it**, not merely the next entry. The
`2026-08-06` entry carries two
...
touches four records nobody got wrong. ⚠ **A 2026-08-11 entry has since been
added at the head of the chain and the same rule covers it and everything under
it.** ⚠ *Counting note, because this banner's own "four" invites one: the chain is
**eight** labelled entries, not four — "four" refers to the specific records this
banner declined to renumber in 2026-08-10, not to the chain's length. Do not read
it as a count of the chain, and do not "correct" it to one; it was accurate about
what it described.*

⛔ **EXTENSION 2026-08-11 — ONE CLAIM IN THIS BANNER'S OWN CHAIN IS FALSE, NOT
MERELY EXPIRED, AND IT PROPAGATED TO FOUR FILES.** Every entry below that speaks
of the container's `RestartCount: 0` since `2026-08-05T16:34:10Z` as *"~5 days of
live uptime evidence that CG-80's rebuild will spend"* is **wrong**. It was not
spent. **`dockerd` on the NAS died on 2026-08-10 and the container did not survive
it.** The streak's last **observed** moment is the NAS soak stream's final sample,
`2026-08-06T22:29:53Z` — roughly **25 h witnessed**, with nothing watching the four
days between that sample and the outage. ⚠ **So CG-82 task 1 is MOOT: it cannot be
discharged. LOST, not SPENT** — spent would have bought a deploy; lost bought
nothing. That distinction is the whole reason the task existed and it must not be
smoothed into *"the rebuild happened, so the capture is behind us"*.

⛔ **AND THE FINDING THAT OUTRANKS EVERY ROW BELOW: the gateway was down 23+ hours
and NOTHING TOLD ANYONE.** This project has a dead-man switch for **tenants'**
silence and no monitor for its **own absence** — and it structurally cannot get one
the same way, because **`/healthz` cannot report that it is not answering.** Filed
as **CG-84**, a row rather than a paragraph. The soak's second death is **CG-85**.

✅ **CG-80 AND CG-59 ARE BOTH IN PRODUCTION AS OF 2026-08-11**, which falsifies the
*"not built is the deploy"* line immediately below and the *"two rows ride the same
next redeploy"* handoff in several places. The redeploy happened (`app.redeploy`
its **first real exercise**), MCP is enabled and **has delivered a real
message with a live hard-rule-#4 refusal beside it**, and the Homepage tile —
which turned out **never to have existed** in the config the NAS serves — was
created pointed at `?strict=1`. **One thing did NOT get verified and is not to be
read as done: `?strict=1` returning 503 on the box.** The record has ONE home,
`docs/deploy/nas.md` §10's **2026-08-11** entry; numbers are deliberately not
copied here.

⚠ **Three exact phrases recur through the chain below and every instance of each
is now FALSE. They are listed verbatim so a reader can match them on sight rather
than having to judge:** *"the surface ships with **no caller**"* — an operator
minted the key and wrote the registry entry on 2026-08-11; *"no real MCP client
has handshaked (so **which era is load-bearing** is still unmeasured), and no
message has reached a real Chat space through the tool"* — **both eras were driven
live and a real message was delivered, so the answer is BOTH** and D4a is settled
by measurement; and *"the **first real exercise** of `app.redeploy`"* stated as a
future bonus — **it ran, and it succeeded.** **The dated entries carrying them are
deliberately not rewritten**, per this banner's standing rule.
...
The entry below says *"Eight decisions are open … do not start CG-80 before they are
answered"* — **answered; that instruction is discharged.** Its other claim, *"Nothing
is built and nothing is decided"*, is now **wholly** false: ~~decided, yes; built, no —
`git diff main -- src/` is still **empty**, re-measured 2026-08-10, and the suite is
**390 passing** measured the same day, not copied.~~ ⚠ **That struck sentence was true
for the first hours of 2026-08-10 and false by the end of it — CG-80 SHIPPED the same
day (PR #72).** `src/chat_gateway/mcp.py` exists, and the suite is **491**. The `390`
survives above as the *measured baseline it was*, which is the only reason a reader
can check the 491.
⚠ **What is still NOT built is the deploy** — see the row, and the paragraph below,
whose one surviving constraint the merge did **not** spend.

⚠ **CG-80 is PULLED FORWARD past CG-59's soak — user decision, 2026-08-10 — and the
reasoning it was given was WRONG in a way worth recording.** The decision was taken
on the understanding that the soak had *barely started*. It had not: measurement
showed the sampler had been **dead for four days**. The decision stands and is
unchanged; **what changed is why it is safe**, and this repo has been burned before
by a right answer kept for a reason that turned out to be false. The sequencing note
CG-80 carried is void — not overruled, but **outlived by the thing it was
protecting**.

⚠ **Three soak findings filed as CG-82**, all measured rather than reasoned: the
sampler died at **24 h 12 m of 72 h**; a **real inbound degradation** (`ConnectError`,
3→6 consecutive poll failures) sat unread in the capture for four days; and
`events_seen` is **`0 → 0` across the entire window**, which `CLAUDE.md` predicts
should have moved. ~~⚠ **One ordering constraint survives all of this:** the container's
`RestartCount: 0` since `2026-08-05T16:34:10Z` is ~5 days of uptime evidence that
**CG-80's rebuild spends permanently** — capture it first (CG-82, task 1).~~
~~✅ **STILL UNSPENT as of CG-80's merge, 2026-08-10 — and that is why this sentence is
in the present tense rather than the past.** CG-80 shipped its **code** and stopped
short of the box precisely so this capture could go first, so the evidence is intact
and the constraint is **live, not historical**. ⚠ It now guards **two** rows' redeploy
rather than one: CG-59's `?strict=1` never reached the box either.~~

⛔ **STRUCK 2026-08-11 — the constraint did not survive, and it did not expire
either. It was DESTROYED by something nobody chose.** `dockerd` on the NAS failed
on **2026-08-10** and the container did not come back with it. The streak's last
**observed** moment is the NAS soak stream's final sample, `2026-08-06T22:29:53Z`
(`restart_count: 0`, `started_at: 2026-08-05T16:34:10.039455732Z`, `status:
running`) — so roughly **25 h witnessed**, never the ~5 days these two sentences
kept asserting in the present tense, and **nothing observed the four days
between**. ⚠ **CG-82 task 1 CANNOT BE DISCHARGED. It is moot — LOST, not SPENT.**
Struck rather than deleted because the shape recurs and is worth recognising: a
claim that reads as *"still true, checked recently"* precisely because it was
re-asserted in the present tense **by a document, on a schedule of its own, with
nothing measuring the thing it described.** The `✅` in front of *"STILL UNSPENT"*
was the tell — a checkmark on a fact nobody had looked at for four days.
✅ **Both rows it guarded shipped to the box on 2026-08-11 anyway**, for their own
reasons; the record is `docs/deploy/nas.md` §10's 2026-08-11 entry.
...
SIGKILLed **one second after a start attempt** on 2026-08-10; systemd then latched
(*"Start request repeated too quickly"*), which is how a one-second failure became
a ~23 hour absence. **`app.query` returned `[]` for all 11 stacks, not only this
one.** No kernel OOM, pool ONLINE at 38 % with a clean scrub, data intact.
Recovered 2026-08-11 with `reset-failed` + `start`; 16 containers returned.
⚠ **What was fixed is the LATCH, not the CAUSE — the original SIGKILL is
unexplained and may recur.** ⛔ **And the finding that outranks it: the gateway was
gone for a day and NOTHING TOLD ANYONE.** This project has a dead-man switch for
*tenants'* silence and no monitor for its own absence — **`/healthz` cannot report
that it is not answering.** Filed as **CG-84**.

⛔ **THE UPTIME EVIDENCE IS GONE, AND FOUR FILES SAID OTHERWISE.** *"`RestartCount:
0` since `2026-08-05T16:34:10Z` = ~5 days of live uptime evidence that CG-80's
rebuild will spend"* is **false**: the container did not survive the outage. Last
**observed** alive `2026-08-06T22:29:53Z` — ~25 h witnessed. **CG-82 task 1 is
MOOT — LOST, not SPENT**, and that difference is the whole of why the task existed.
Corrected in the queue (row **and** detail body), the spec, the plan and
`nas.md` §9/§11; **history annotated, not deleted**.

✅ **CG-80 IS DEPLOYED AND EXERCISED — it is no longer *"merged but not in
effect"*.** Redeploy `52710df` → `1f2d886`, `app.redeploy` job **3112** SUCCESS —
its **first real exercise**, so nas.md §10 deviation 6 stops saying *untested*.
An operator minted a key (over stdin, never argv), added `agent-mcp` to the
gitignored registry with **one** identity, and turned the surface on
(`app.update` job 3180). Live: **401** unauthenticated, **both eras** negotiate,
`cacheScope: 'private'` **confirmed on the wire**, a **real message delivered**
through `tools/call`, hard rule **#4 refused `aitrader-alerts` live**, and the
delivery-log row is there — with **no row for the refusal**, which is correct.
✅ **This ANSWERS the plan's own *"single most valuable"* open question — which era
is load-bearing. Both.** D4a is **vindicated by measurement**; stop carrying it as
open. ⚠ **`aitrader-*` was deliberately NOT granted, and the identity list is the
only control there is** — spec §14 **D7** declined an `allow_mcp` flag, so that
was an act, not a default.

✅ **CG-59's `?strict=1` reached the box in the same redeploy** — and the
**422 turned out to be a technique**: the old image did not declare `strict` so it
answered 200 to anything, the new one answers **422 to `?strict=banana`**, which
identifies the live image **in one request without degrading production**.
⛔ **Still NOT verified on the box: `?strict=1` returning 503.** A genuine degraded
window occurred after the restart and cleared before it could be sampled; the 503
path is proven only in CG-59's driven local test. ⚠ **The Homepage tile did not
exist** — `services.yaml` carried 8 NAS tiles and this was not one of them, so
§9's *"repointing the tile"* hazard described a tile nobody had created. Created
2026-08-11 on `?strict=1`; **`capture.sh` has not been re-run**, so the homelab
repo does not record it.

✅ **The soak's real numbers are final and one of them is good news:** the NAS
memory stream ran **25.2 h / 152 samples** and was **FLAT** — `peak_bytes`
**unchanged**, 6 threads and 9 fds constant. **No leak.** ⚠ **Over 25 h, not the
72 h every plan assumes.** ⚠ **Both streams died the same evening ~90 minutes
apart on two different hosts** — an unexplained common cause, recorded as an open
question and filed as **CG-85**, whose real subject is that this project has now
lost two runs for want of a capture that survives a host event.

⚠ **No ⚠ verification-ledger flag moved, and this was the day to want one.** A
message delivered through the MCP tool is **the same webhook bytes from a
different caller**; `webhook.send` cleared 2026-07-29 and nothing extends. Suite
**491**, both ends re-measured with `python3 -m pytest -q`. Docs only — `git diff
main -- src/` is empty.

⚠ **Pre-merge review found 0 HIGH, 2 MEDIUM, 1 LOW — and the MEDIUMs were this
file's own recurring defect, committed by the PR that was correcting it.** Both
were **one-home violations in the new text**: the redeploy's **job ids** had been
copied into the MCP spec and plan and repeated three times inside CG-80's detail
section, and the container's new **nanosecond `StartedAt` baselines** had been
copied into `nas.md` §9, §11 **and** CG-82's row while appearing **nowhere in
§10**, the very section named as their home. ⚠ **The second is the sharper one:
the home did not hold the fact, so every "one home" pointer in the PR pointed at
a section that did not have it.** Fixed — §10 now carries the baselines and the
other three sites point at it; the Planner artifacts carry no run identifiers at
all, because a spec records *what a decision cost*, not *which job id ran*. The
LOW was a sentence that restated the soak's headline numbers in the same breath
as saying it would not; reworded to separate the **verdict** (which the row must
be read against) from the **byte-level series** (which it must not copy).
⚠ **Recorded rather than quietly fixed, because it is evidence for CG-69:** a PR
whose entire subject is *"a number with two homes drifts"* introduced two of
them, and neither was caught by the author. Previous entry follows.)

**Previously:** 2026-08-10 (Builder — **CG-80 SHIPPED as code, PR #72 — and
deliberately NOT deployed.** `POST /mcp` is on `main`: dual-era, send-only, one
generated-schema tool, behind `GATEWAY_ENABLE_MCP` (default OFF). Suite **390 →
491**, both ends measured. **No ⚠ ledger flag moved** (`grep -c` → 0).

⚠ **The rebuild in the plan's Task 8 step 6 was excluded from scope**, because its
own ordering constraint — **CG-82 task 1's `docker inspect` capture** — is not
discharged and reaching the box was not authorized. So **two** rows now ride the
same next redeploy, CG-59's `?strict=1` and this one; that heavier handoff is
recorded in `docs/deploy/nas.md` §9 hazard 2, where the person doing it will read
it, rather than only in a queue row.

⚠ **Three findings worth carrying up out of the row:** the modern header
cross-check was **bypassable and the bypass DELIVERED a message**; the blocking
send ran **on the event loop**, so one hung webhook would have stalled the
unauthenticated `/healthz` this project's hard rule #5 exists to keep honest; and
**the rule #1 guard was blind** — six candidate hand-edits, six undetected,
including the two the spec names as *the* leak vector. All fixed and re-measured.
⚠ **The guard is the one to remember:** this row's whole claim about hard rule #1
was that the answer is *mechanical rather than tasteful*, and the mechanism did
not work. It was reassured by having caught a `cards.items` edit, which was simply
the wrong test to be reassured by.

⚠ **The plan's own code contradicted the plan's own Goal** — `call_tool` wrote
nothing to the delivery log while spec §2 and the Goal line both promised audit
journalling, and Task 8's UAT asked for the resulting row as evidence. Found in
build, not review. ⚠ **UAT has two named gaps that merging does not close:** no
real MCP client has handshaked (so *which era is load-bearing* is still
unmeasured), and no message has reached a real Chat space through the tool. Detail
in § CG-80. Previous entry follows.)

**Previously:** 2026-08-06 (Planner — **CG-80 filed: the gateway becomes an MCP
server, send-only. CG-81 filed and deliberately NOT queued.**

**Nothing is built and nothing is decided.** Spec + plan + these rows only; `git
diff main -- src/` is **empty**. **Eight decisions are open in
[spec §14](superpowers/specs/2026-08-06-mcp-server-surface-design.md)** and three
of them change what gets built — do not start CG-80 before they are answered.
Three *were* settled by the user in the brief (audience is *both*; inbound scope
and process model were explicitly **delegated to the Planner**), so D1 and D2 in
that table are answers rather than re-asks.

⚠ **The protocol broke nine days before this row was written, and it resized the
work.** MCP revision `2026-07-28` is not a point release: it **deletes**
`initialize`, `notifications/initialized`, `ping` and `Mcp-Session-Id`, and adds
`server/discover`, per-request `_meta`, three required headers and
`resultType`/`ttlMs`/`cacheScope`. **Modern client → legacy server fails, and
legacy client → modern server fails** — so serving one era makes the endpoint
**silently unreachable** to clients speaking the other. Filed as **D4a**,
recommendation dual-era. ⚠ **The spec's own sizing section was drafted before
this finding arrived and now SAYS so** rather than leaving the earlier "one PR,
comfortably" standing — CG-69 category (b), caught in self-review rather than by
a reader.

⚠ **Two findings that would have shipped a hard-rule-#4 breach, both found by
measuring rather than reasoning.** (1) **`cacheScope` must be `"private"`**:
`"public"` asserts a response may be cached *across authorization contexts*, and
this server's tool list **does** vary by API key because `identity`'s enum is
that app's allowlist — so `"public"` would let an intermediary serve one tenant's
identity allowlist to another. **A rule #4 violation delivered by a cache
header**, invisible to a review looking at the auth check. (2) The schema
narrowing **mutates** the dict `model_json_schema()` returns; had pydantic handed
back a **cached** one, an app's enum would persist into the next caller's schema
*and* into `/openapi.json`, which serves the same model to every unauthenticated
reader of `/docs`. **Measured: it does not** — and a test now pins it, because
the failure mode is silent, global, and one upgrade away.

**Hard rule #1's answer is mechanical, not a reassurance.** A tool `inputSchema`
is a schema and a tool `description` is a **prompt**, so both are where app-domain
knowledge leaks in — invisibly, because it reads as helpfulness. The
discriminator is whether a human typed a property name, so the schema is
**generated** and a test compares it for equality against a fresh generation.
⚠ **Verified during planning that the guard CATCHES a hand-edit to
`cards.items`** — a guard that does not is worth nothing, and this repo has
shipped one of those before.

⚠ **CG-81 is filed and NOT queued, on a reason that is not a policy question.**
`Inbox.poll` **drains**, so until CG-56 lands an MCP `read_inbox` and a tenant's
production poller are **competing destructive consumers of one queue** — a model
silently deleting replies jobhunt was about to collect, undetectably, because the
audit trail records what **arrived**, never what left. The rule #6 analysis is
argued **both ways** in spec §7 rather than resting on *"it is gated on the
caller's own `allow_inbound`"*; it needs the user's explicit sign-off **naming
that rule**, and it is coupled to CG-80's D2 in a way that makes choosing the
mounted route **sharpen** the question rather than settle it.

⚠ **Sequencing — RESOLVED 2026-08-10 by the user: CG-80 goes first.** This
paragraph read *"and it is not this row's to invent"*, which was right, and the
constraint it described has since expired rather than been overruled. CG-80 still
needs a rebuild + `app.redeploy`, which restarts the container. What has changed is
that the `/healthz` sampling that uptime was protecting **stopped on
`2026-08-06T21:02:42Z` after 24 h 12 m** (CG-82), and the NAS stream's own 72 h
ceiling from `2026-08-05T16:34:10Z` has also passed. ⚠ **One piece of it is NOT
expired and CG-80 must not spend it blind:** `RestartCount: 0` across ~5 days is
live evidence right now, and the rebuild destroys it. **CG-82 task 1 captures it
first**; that is the single ordering constraint between these two rows, and it is
one `docker inspect`, not a wait. ✅ Two bonuses fall out: that redeploy is the **vehicle that finally
lands CG-59's `?strict=1` on the box** (order per `docs/deploy/nas.md` §9 hazard
1 — rebuild, verify 503 on a degraded boot, *then* repoint the tile), and it is
the **first real exercise of `app.redeploy`**, which nas.md §10 deviation 6
records as documented-but-untested.

✅ **No new deploy artifact and no new secret** — checked against the runbook
rather than assumed. `GATEWAY_ENABLE_MCP` holds no credential, so there is no new
`.env` key, no new stdin transport and **no new `SECRETS.template.md` row**;
nas.md §6's three-row set is unchanged. ⚠ **But merging is not finishing:** the
surface ships with **no caller** until an operator mints a key and edits the
**gitignored** `config/registry.yaml`. CG-61's lesson exactly.

⚠ **No ⚠ verification-ledger flag is cleared, added or reworded** — this PR
touches no `src/` and no `tests/`, and the spec §11 states the position in the
ledger's own terms rather than by silence, **including naming the temptation a
Builder will meet**: a live round-trip through the MCP tool will *look* like
fresh Google evidence and is **not**. Suite **390**, measured on `f4b9c99` with
`python3 -m pytest -q`, not copied from `CLAUDE.md`. Previous entry follows.)

**Previously:** 2026-08-05 (Builder — **CG-59 half 1 SHIPPED, half 2 RUNNING.**
...
**And the one before that:** 2026-08-05 (Planner — **CG-59 REFRESHED + CG-79's routed list
...
and HELD … not merged"*, and **it was not the only one — the sweep corrected MORE THAN
FORTY passages across seven files, on SIX falsified facts**, including three
that were already false *before* the deploy ran. ⚠ *A precise site count was
drafted and then deliberately dropped: two of this row's own numbers were wrong
on first writing (see the row), and a tally nobody can re-derive is exactly what
this row exists to stop shipping. The floor is checkable; the point was never the
number.*
**CG-61's live-registry operator action is DONE** — measured through the real
`load_registry`, `aiteam-harness allow_inbound=False` with the key now
**explicitly present** at `config/registry.yaml:38`, file mtime
`2026-08-03T20:34:06Z`. That is **an hour and eighteen minutes after** the
`2026-08-03 19:15:56Z` measurement three rows still quote as the current state,
and `docs/deploy/nas.md:609` recorded the pre-flight PASS on 2026-08-05 — so the
fact was **already written down in this repo, in a different file, while this
file went on calling it outstanding**. `capture.sh` has been **run** and its
output verified clean by the user; the artifact lives in the **homelab** repo and
is named, not reproduced. ⚠ **`iac/chat-gateway-sa.json` is DELETED** — the
condition CG-55's row set for rewording the dead-key warnings, so the five
present-tense *"still present"* warnings are reworded, **not dropped**, exactly as
that row instructed — and each now warns against the dead key's **shape** rather
than a path this repo has deleted, because the setup scripts still default to that
filename and copies outlive this working tree.
⚠ **A SIXTH fact, folded in by user approval and worse than the other five: the
gateway is the ELEVENTH stack, not the tenth.** `docs/deploy/nas.md` §1's
**heading** said tenth while its own **body** three lines down measured 10
pre-existing stacks; the live `app.query` lists **11**. **Wrong when written —
CG-69 category (b), not (a)** — and it propagated by quotation before a sibling
agent caught it by counting rather than by trusting its brief. Corrected in six
places.
⚠ **This row got a number wrong in its own draft too**, and **deleted** it rather
than fixing it: the captured config's env-var count is **six**, not the four a
too-narrow grep produced. That count lives in the homelab repo and no longer has a
second home here.
⚠ **No ⚠ verification-ledger flag cleared, added or
reworded**: `git diff main -- src/ | grep -c "LIVE-UNVERIFIED\|SHAPE-VERIFIED"`
→ **0**, and the one `src/` hunk is a comment `CLAUDE.md` names as this edit's
paired site. ⚠ **The `docs/superpowers/` specs and plans carry the same claims and
were NOT edited** — a Builder does not rewrite a Planner artifact — but they are
**routed with file and line**, re-checked by `grep` here rather than copied,
because CG-69 instance 5 is precisely the case where staying in lane shipped a
false claim anyway. ⚠ **Logged as CG-69 evidence instance 6** — a merge going stale in
...
the user's review; not merged.**~~ ⚠ **CORRECTED 2026-08-05 (CG-79): that PR is
[#66](https://github.com/mmackelprang/chat-gateway/pull/66) and it MERGED the
same day as `4ddd6f5`** (`gh pr view 66` → `MERGED`, `2026-08-05T18:04:05Z`).
Struck rather than deleted, because **this sentence is the whole reason CG-79
exists**: it is the first line of the file a Builder reads, it was true for a
matter of hours, and nothing about it looked stale. ~~⏸ `capture.sh`
outstanding~~ — ⚠ **also corrected: it has been RUN by the user and its output
verified clean** (that artifact is the **homelab** repo's, named in CG-79's row
and not reproduced here). ⏸ homelab §9 artifacts outstanding, tailnet ACL still
...
⚠ **Pre-merge review found a HIGH the plan never considered, and it would have
produced exactly the failure this row exists to prevent.** `parse_env_file`
stripped whole-line `#` comments but **not inline ones** — and `.env.example`
uses inline comments on **11 of its 18 keys**. Measured through the real parser:
`GOOGLE_APPLICATION_CREDENTIALS` came back as `'# path to the service-account key
JSON (mode 600)'` and `CHAT_GATEWAY_PUBSUB_SUBSCRIPTION` as
`'# projects/<PROJECT_ID>/…'`. Both are **non-empty**, so `__main__`'s
fail-closed guard (`if not sub or not creds: raise RegistryError`) did **not**
fire and `registry.health()` — which tests `bool(os.environ.get(...))` — reported
the credential **resolvable**. The gateway would have booted, looked alive on an
unauthenticated `/healthz`, and been unable to talk to Google: the exact shape
`env_file.py`'s own *"a missing file is FATAL"* docstring argues against, reached
by a path that never raises. It also silently defeated the
`GATEWAY_ENABLE_PUBSUB` fail-closed lever that runbook §5 declares in the compose
*for that very reason*. Found only because the reviewer ran the real parser over
this repo's **real** `.env.example` instead of the plan's synthetic fixtures.
Fixed to Compose's documented rule — the same file is read by **both** parsers
(dev box via `env_file: .env`, box via this loader), so divergence is what makes
a working file change meaning at the destination. Pinned by a test that parses
the repo's own `.env.example` and asserts **zero** values contain `#`.
...
loader at **2026-08-03 19:15:56Z (15:15 EDT)** and **not yet done**.~~
⚠ **EXPIRED, corrected 2026-08-05 (CG-79). The operator edit was made the same
day that measurement was taken** — `config/registry.yaml` mtime
`2026-08-03T20:34:06Z`, **78 minutes later** — and re-measuring through the real
`load_registry` returns `aiteam-harness allow_inbound=False` with the key now
**explicitly written** at line 38, not defaulted. So this "currency pointer" was
stale within the hour and stayed the file's designated current answer for two
days. **The lesson is the label, not the fact:** a sentence that calls itself the
currency pointer is trusted *more*, so it ages *worse*. The row's own advice —
*"re-measure rather than trusting this line"* — is the part that worked. Deferred is
...
⚠ **That "second half is NOT stale" judgement has itself EXPIRED — corrected
2026-08-05 (CG-79), and it is worth more than the fact it corrects.** This
paragraph split one banner into a stale half and a durable half and was right on
both counts *on the day*. What it could not do is stay right: the durable half
was falsified on **2026-08-03** by an operator action taken in a gitignored file,
which is the one class of change no diff, no test and no reviewer in this repo
can see. **A sentence explicitly certified not-stale reads as verified rather
than as merely current**, which is exactly why it sat unexamined for two days.
Both halves are now spent; the paragraph is kept whole because the pattern is the
finding.
...
file, not on the example**.~~ ⚠ **DONE — the operator made the edit
2026-08-03T20:34:06Z** (file mtime), and re-measuring through the real
`load_registry` on 2026-08-05 returns `aiteam-harness allow_inbound=False` with
`allow_inbound: false` now **written explicitly** at `config/registry.yaml:38`
rather than defaulted. `aiteam-harness` is **no longer open inbound in
production**; CG-55's own fail-closed pre-flight independently confirmed it on
the box (`docs/deploy/nas.md:609`). The PR body carries the edit and its
...
is an *additional stack*, not a role change. *(This said "a **tenth** stack".
⚠ **Wrong when written, not gone stale** — 10 were already running, so the
gateway is the **eleventh**, and the live `app.query` captured 2026-08-05 lists
**11 apps**. The measurement in the same sentence was always correct; only the
ordinal derived from it was not. Corrected 2026-08-05, CG-79; the full account is
in `docs/deploy/nas.md` §1, which carried the same contradiction in its own
heading.)* Probed read-only: **20.8 GB RAM free,
...
| **CG-61** · close `aiteam-harness`'s inbound path (decision D1) | ✅ **done — merged (#50) AND the operator action complete** (2026-08-03) | Gate released by the user; merged 2026-07-31 as `dced002`. ⚠ **Merging is not finishing** — the PR changed only `registry.example.yaml`; the **live gitignored registry edit was a separate operator action**, and **CG-55 depended on the live file**. ~~⚠ **Still outstanding — measured, not assumed:** `load_registry("config/registry.yaml")` at **2026-08-03 19:15:56Z** returned `aiteam-harness allow_inbound=True`, and the `allow_inbound` key is **absent** from that app's block (file mtime `2026-07-30T15:49:24Z`), so the `True` is the loader default — exactly D1's reasoning.~~ ✅ **DONE — corrected 2026-08-05 (CG-79).** The edit landed at mtime `2026-08-03T20:34:06Z`, **78 minutes after** the measurement struck above; re-measured through the real `load_registry` on 2026-08-05, `aiteam-harness allow_inbound=False`, with `allow_inbound: false` now **explicitly written** at line 38 rather than defaulted. CG-55's fail-closed pre-flight confirmed it independently on the box (`docs/deploy/nas.md:609`, 2026-08-05). ⚠ **This row's own instruction — *"re-measure rather than trusting this line"* — is the only reason it did not ship a false gate**; the line was already wrong when CG-55 read it. Suite 246 → 247. Plan Part I |
| **CG-53** · deployment artifacts + secret-safety proof (**no deploy**) | ✅ done (#65) | ✅ **merge gate RELEASED by the user 2026-08-03** — secret-handling path; the row states exactly what releasing it approved (four things, **none of them a deploy**). ⚠ *This cell read* ***"held, the user's to release"*** *until 2026-08-05 while the row's own detail section three thousand lines below already said* ***"✅ RELEASED by the user 2026-08-03"*** *— a status word with two homes, disagreeing. Found by CG-79's sweep for a different row and corrected rather than left, because the sweep is the only thing that would have found it.* ✅ **The premise was RE-MEASURED, not re-quoted** — it is the one load-bearing fact here and it lives in a repo this one does not control: through the homelab redactor's **real** `is_secret_key`, all seven credential vars still miss, and end to end through the **real** scan gate a payload keeping a live-shaped API key and webhook URL **exited 0**. ⚠ One correction: *"the `__<SUFFIX>` convention defeats it"* is only half right — bare `GOOGLE_CHAT_WEBHOOK_URL` is missed **anyway** (no `URL` suffix), so **renaming would not fix the webhook family**, the one secret with no rotate-in-place. ⚠ **Ten drifts found and corrected in Part A** across NINE merged PRs — **two would have failed at runtime**: A2's `build_runtime()` unpack is now a **6**-tuple (CG-68's `sweeper`), and §5's compose mounted the registry where its own env var did not point. Also folded in: **CG-70's owed runbook line** (that row stays **open**), the real state tree (**four** directories hold tenant bodies; `quarantine/` is never swept), **D2's deferral** replacing an *"ACL lands BEFORE deploy"* claim, and `202`→345→**359** (CG-76 landed mid-refresh and moved it a second time). Plan Part A |
...
| **CG-55** · first NAS deploy + live smoke | ✅ **done — DEPLOYED and serving** (2026-08-05) · **[PR #66](https://github.com/mmackelprang/chat-gateway/pull/66) MERGED** as `4ddd6f5` (⚠ this cell read *"PR open, ⏸ HELD for the user's review before merge"* until 2026-08-05; the merge happened hours later the same day and the cell did not move — **CG-69 evidence instance 6**) | ✅ **The gateway runs on the NAS. All five facts observed, §5's three open verification points answered on the box, seven deviations recorded — and the record has exactly ONE home: `docs/deploy/nas.md` §10 *Executed*.** Deliberately not summarized here; the interesting numbers are the ones that will move, and this file is where a second copy of a moving fact has burned this project three times. The row's detail section carries only what is still **open** and who owns it. ~~⚠ **`capture.sh` NOT run — 🛑, requested, outstanding.**~~ ✅ **RUN by the user and its output verified clean — corrected 2026-08-05 (CG-79).** The captured JSON lives in the **homelab** repo; it is **named, not reproduced here**, because it is that repo's artifact and a second copy of it in a public repo is the thing the capture gate exists to prevent. ⚠ **Still open: the homelab §9 artifacts** (a PR is being prepared in that repo) **and CG-59's soak.** ⚠ **No ⚠ ledger flag cleared, added or reworded** (one candidate named, left for CG-59). ✅ **merge gate released — PR #66 merged 2026-08-05.** **Builder-executed over SSH**. Depends on CG-53, CG-54, **CG-61** (✅ live registry edit done 2026-08-03), **CG-75**, **CG-76** (⚠ **the CG-76 dependency was NOT recorded on this row until 2026-08-03** — the user's 2026-08-02 pre-deploy-blocker decision existed, and `grep` found it named nowhere under either CG-55 entry. A first deploy whose dead-man switch has six ways to drop an alert with `/healthz` green is a first deploy that should not happen, and aitrader's contract surface is exactly that switch), **CG-75** (⚠ **added 2026-08-03, user decision** — this gateway has never run on a box with a real disk that can fill, so CG-75's *"low likelihood"* is an artifact of never having been deployed, and **CG-55 is the event that changes it**; a first deploy that can turn a full disk into an unbounded send storm against Google is a first deploy that should not happen), and ~~⚠ an **external homelab-repo prerequisite (D2)**~~ — ⏸ **D2's tailnet ACL is DEFERRED by the user, 2026-08-03. It no longer gates this row; the dependency is recorded, not deleted.** ⚠ **Paired with a second decision the same day that this row must BUILD: bind the published port to the LAN interface, not `0.0.0.0`** (both, with reasoning and residual, in the row). Part C |
...
| **CG-59** · long-run observation + a deployed `/healthz` | 🚀 **in flight** (2026-08-05) · ✅ **half 1 SHIPPED** ([PR #70](https://github.com/mmackelprang/chat-gateway/pull/70)) **and LIVE ON THE BOX since 2026-08-11** · ⚠ **half 2 DIED AT 24 h — measured 2026-08-10, see CG-82** (this cell read *"⏳ half 2 RUNNING"* for four days after it stopped) · ⛔ **both streams now final; the uptime evidence is GONE** | ~~Depends on **CG-55** — the soak clock starts when it lands.~~ ✅ **CG-55 landed 2026-08-05 and the soak clock started with it** — corrected 2026-08-05 (CG-79); the future tense here survived the event it was waiting for. ✅ **`GET /healthz?strict=1` shipped 2026-08-05** — 503 when `reasons` is non-empty, byte-identical body, plain form untouched; driven against a real server and compared by sha256, not asserted. ✅ **AND IT REACHED THE BOX 2026-08-11**, on CG-80's redeploy exactly as planned; byte-identity re-confirmed there. ⚠ **The 422 turned out to be a TOOL:** the old image did not declare `strict` so it answered 200 to any value, the new one answers **422 to `?strict=banana`** — a **one-request discriminator for which image is live, without degrading production**. ⛔ **But `?strict=1` → 503 has STILL never been seen on the box.** A genuine degraded window occurred after the restart and cleared before it could be sampled; the 503 path is proven only in the driven local test. ⚠ **The Homepage tile was never REPOINTED because it never EXISTED** — `services.yaml` on the box held 8 NAS tiles and this was not one of them, so hazard 1's whole framing described a tile nobody had created. Created 2026-08-11 on `?strict=1`, with a comment saying why the `1` is load-bearing; **`capture.sh` not re-run**, so the homelab repo does not record it. ~~⏳ **The soak is running on the box** (both streams; §11 of the runbook)~~ ⚠ **FALSE since 2026-08-06T21:02:42Z, corrected 2026-08-10 (CG-82).** The dev-box `/healthz` stream ran **24 h 12 m of 72 h** and stopped; the NAS memory stream's own 72 h ceiling has also passed. ✅ **The NAS stream's numbers are now IN and one of them is good news — 25.2 h, 152 samples, FLAT:** `peak_bytes` **unchanged**, 6 threads and 9 fds constant, `restart_count: 0` throughout. **No memory leak, no fd growth, no thread growth** — a clean answer to the question this half existed to ask, over **25 h rather than 72 h**, which is not the same run and must not be reported as one. ⛔ **The `SubscriberLoop` evidence path took a hard hit, though:** the container's uptime streak was **destroyed by the 2026-08-10 `dockerd` outage**, not spent by a rebuild — last observed alive `2026-08-06T22:29:53Z`, ~25 h witnessed — so this row's best single input to that ⚠ flag is gone and **CG-82 task 1 is moot**. The row stays 🚀 until its analysis, the disk-growth calibration and the ledger **proposal** land — **against 25 h, not the 72 h every plan for it assumes**, and CG-82 carries what the `/healthz` half contains (including a degradation nobody read). This row is still the **only** thing standing between the `SubscriberLoop` long-run ⚠ flag and a decision about it — ⚠ **and neither shipping half 1 nor deploying it moved that flag at all**, which is the point. **A re-run needs CG-85 first.** Part G |
...
| **CG-69** · published-promise inventory (process control) | 📋 queued · ⚠ **evidence list now at SIX** · ✅ **designed 2026-08-03** | ⚠ **Instance 6 added 2026-08-05 by CG-79's Builder** — the queue's own banner said CG-55's PR was *"open and HELD … not merged"* while it had merged hours earlier, and **more than forty sibling passages said the same or worse**; category (a), and the first instance whose falsified claim is about **this file's own bookkeeping** rather than about code. Filed by CG-65's Planner. Three changes in one day invalidated a guarantee recorded in a file nobody in the loop was reading. ⚠ **Instances 4 and 5 added 2026-08-03 by CG-76's Planner** — #4 CG-74 falsifying its own spec's `/healthz` strings, #5 CG-76 finding the delivery write-path spec §5's *"the only thing standing between a silently-dropped aitrader alert and a green `/healthz`"* to be false. **Every one was caught only because somebody independently went looking**, and #5 took two independent sweeps of the same code path to find its last two doors. ⚠ *That entry ended "**Still no plan**" when it was written; this PR falsified it hours later and it is corrected rather than left standing.* **Designed the same day — inverted and one fifth the size the row proposed**, because measurement falsified the row's stated cause (**instances 4 and 5 are both inside the sweep's own list**, and the sweep found counterexamples where the diff *did* contain the sentence) and showed the row's proposed pairing has already rotted: **8 of the 14 code citations in the live contracts point at the wrong code**, three of them in `aitrader.md`'s hard-rule-#6 enforcement table, while the same files' name-anchored citations are 0 of 8 wrong. ⚠ **THOSE THREE WERE REPAIRED IN PASSING BY CG-88 ON 2026-08-31 — so the "8 of 14" is stale as a CURRENT count and stands as the 2026-08-03 measurement.** ⚠⚠ **This row is NOT discharged and is not even narrowed in the way that matters: a passer-by fixing three citations because it happened to be editing that table is exactly the ad-hoc repair this row exists because of.** The other five are untouched, nothing re-measures any of them, and CG-88's own PR moved sixteen `registry.py` citations TWICE inside one branch — the second time caused by the commit that fixed something else — which is fresh evidence for this row's design finding that **line-anchored citations rot and name-anchored ones do not.** [spec](superpowers/specs/2026-08-03-published-promise-inventory-design.md) · [plan](superpowers/plans/2026-08-03-published-promise-inventory.md) |
...
| **CG-80** · the gateway becomes an **MCP server** — send-only | ✅ **DONE — code shipped 2026-08-10** (PR #72) **and DEPLOYED, ENABLED AND EXERCISED ON THE BOX 2026-08-11** · ~~⚠ NOT DEPLOYED, deliberately — the row is not ✅ on the box~~ · ✅ specced + planned 2026-08-06 · ✅ all 8 decisions signed off at the recommendation, 2026-08-10 (spec §14) | ✅ **IN EFFECT 2026-08-11, so the *merged ≠ in effect* caveat below is DISCHARGED rather than deleted.** Redeploy `52710df` → `1f2d886`, `app.redeploy` job **3112** SUCCESS (its **first real exercise**, so nas.md §10 deviation 6 stops saying *untested*), `GATEWAY_ENABLE_MCP` set via `app.update` job **3180**. An operator minted a key **over stdin, never argv** (rule #2) and added `agent-mcp` to the gitignored registry. Live and measured: **401** unauthenticated and on a wrong key; **legacy `2025-11-25`** negotiates from a bare `initialize`; **modern `2026-07-28`** demands three headers **and** two `_meta` keys, each omission its own specific 400; `server/discover` **200** with **`cacheScope: 'private'` confirmed on the wire** — the rule-#4 control the spec flagged as a leak vector, verified where it matters; **a real message DELIVERED** through `tools/call`; **hard rule #4 refused `aitrader-alerts` live**; and the `GET /v1/deliveries` row is present with **no row for the refusal**, which confirms CG-80's build-time `DeliveryLog` fix in production. ✅ **This ANSWERS the plan's own *"single most valuable thing this task produces"* — which era is load-bearing. BOTH.** Legacy is reachable with a bare `initialize`; modern is not reachable without five separate pieces. **D4a is vindicated by measurement, not by argument — stop carrying it as open.** ⚠ **`aitrader-alerts`/`aitrader-reports` were deliberately NOT granted**, and since **D7 declined an `allow_mcp` flag the identity list IS the only control** — so that was an act, not a default, and adding an identity here later widens a real-money tenant's model-addressable surface. ⚠ **No ⚠ ledger flag moves and this was the day to want one:** a send through the tool is the **same webhook bytes from a different caller**; `webhook.send` cleared 2026-07-29. Record: `docs/deploy/nas.md` §10, **2026-08-11**. **Filed by Planner 2026-08-06** from the *inverse* of the question asked: consuming a third-party Chat MCP server helps this project not at all; **exposing** one is additive. `POST /mcp` is another ingress to the send path — same `authenticate()`, same `registry.identity_for`, same adapter — so hard rule #4 holds **by reuse**. ⚠ **The protocol broke nine days before this row:** `2026-07-28` deletes `initialize`, `ping` and sessions, and **modern↔legacy fails in BOTH directions**, so picking one era makes the endpoint **silently unreachable** to the other (decision **D4a**, recommend dual-era — and it roughly doubles the protocol layer, which §16 now says rather than leaving its pre-finding estimate standing). ⚠ **`cacheScope` MUST be `"private"`** — the tool list varies by API key, so `"public"` lets an intermediary serve one tenant's identity allowlist to another: **a rule #4 leak delivered by a cache header.** Hard rule #1 gets a **mechanical** answer — the tool `inputSchema` is *generated* from `OutboundMessage.model_json_schema()` and a test compares it for equality; verified during planning that the guard **catches** a hand-edit to `cards.items`, because a guard that does not is worth nothing. Rule #5: **one config echo, no counters**, never an input to `status` — a synchronous route has no silent-failure mode, and the field exists for CG-59's just-learned lesson that a deployed image may not be the one you think. ⚠ **No ⚠ ledger flag moves**, and the spec names the temptation: a live round-trip through the tool *looks* like fresh Google evidence and is **not**. ~~⚠ **Sequenced after CG-59's soak** (a rebuild spends the uptime the soak is measuring)~~ ✅ **PULLED FORWARD — user decision 2026-08-10.** The soak's sampling died at 24 h (CG-82), so the constraint expired rather than being overruled; ~~**one piece of it survives** — capture `RestartCount`/uptime via CG-82 task 1 *before* the rebuild~~ ⛔ **and the surviving piece was DESTROYED before it could be captured — corrected 2026-08-11.** `dockerd` died on 2026-08-10 and took the container with it; **CG-82 task 1 is moot, LOST rather than SPENT.** The rebuild this row was told to sequence carefully around had, by the time it ran, nothing left to spend. ✅ Its redeploy **was** the vehicle for CG-59's `?strict=1`, which landed with it — order per nas.md §9 hazard 1, followed, ⚠ **except for the step that could not be run: `?strict=1` → 503 was never observed on the box.** ~~⚠ **Merging is not finishing:** the surface has **no caller** until an operator mints a key and edits the gitignored registry (CG-61's lesson).~~ ✅ **DISCHARGED 2026-08-11 — the operator did exactly that**, and CG-61's lesson is kept rather than deleted because it was right for a day and its shape recurs. [spec](superpowers/specs/2026-08-06-mcp-server-surface-design.md) · [plan](superpowers/plans/2026-08-06-mcp-server-surface.md) |
...
| **CG-82** · the CG-59 soak **died at 24 h**, and it caught an unread incident | 📋 **queued** · ⚠ **three findings, all measured 2026-08-10** · ⛔ **TASK 1 IS MOOT — corrected 2026-08-11** | ⛔ **TASK 1 CANNOT BE DISCHARGED, AND THE REASON IS THE POINT OF THE TASK.** This row's first task was one `docker inspect` to capture `RestartCount`/`StartedAt` **before CG-80's rebuild spent it**. `dockerd` on the NAS failed on **2026-08-10** and the container did not survive; the rebuild never got the chance. The streak's last **observed** moment is the NAS soak stream's own final sample, **`2026-08-06T22:29:53Z`** (`restart_count: 0`, `started_at: 2026-08-05T16:34:10.039455732Z`, `status: running`) — **~25 h witnessed, not the ~5 days this row asserts below**, with nothing watching the four days in between. ⚠ **LOST, not SPENT. Say which.** Spent would have been a deliberate trade for a deploy — the exact bargain this row was designed to price. Lost means the evidence went and the project got nothing, and *nobody noticed for a day* (**CG-84**). The container's new baselines have **one home and it is not this row** — `docs/deploy/nas.md` §10's 2026-08-11 entry — ⚠ **and whatever they read, they are hours old and not continuous with anything.** *(Pointer rather than a copy on pre-merge review's MEDIUM finding: a **current-uptime** figure is the most moving number this project has, and it is what this very row was filed about.)* ✅ **The NAS memory stream's numbers are now final and belong beside findings (1)–(3):** 152 samples, `2026-08-05T21:15:58Z` → `2026-08-06T22:29:53Z`, `soak_elapsed_s: 90835.9` = **25.2 h of 72 h**, `current_bytes` 52 539 392 → 52 793 344, **`peak_bytes` 54 362 112 UNCHANGED**, 6 threads and 9 fds constant. **Flat — no leak.** ⚠ **A fourth finding, recorded as an open question rather than a conclusion: both streams died the same evening ~90 minutes apart** (dev box `2026-08-06T21:02:42Z`, NAS `2026-08-06T22:29:53Z`) **on two different hosts.** Two independent samplers stopping that close together suggests a **common cause, and it is unexplained**; the per-host inference this row offers for finding (1) does not explain the coincidence, and having two separate stories for one coincidence is how a coincidence gets treated as explained. **Not** connected to the 2026-08-10 `dockerd` failure by anything but suspicion — that is four days later. **Filed forward as CG-85**, because the consequence is bigger than the observation: this project has now lost **two** long-run runs for want of a capture that survives a host event. **Filed 2026-08-10 by the Coordinator, from measuring `~/cg59-soak/healthz.jsonl` on the dev box rather than reading what the queue said about it.** ⚠ **(1) THE SAMPLER IS DEAD AND WAS NEVER 72 h.** Window `2026-08-05T20:50:54Z` → `2026-08-06T21:02:42Z` — **24 h 12 m, 2886 samples, ~34 % of target** — file mtime four days cold, **no sampler process running**. Two places assert it is still accruing (CG-59's row, `nas.md` §11); **both corrected in this row's PR**, because a claim that silently expires is CG-69's whole subject. The likely cause is already documented as a known limitation: the capture is a bare background process with **no systemd unit and no cron** (both on the standing rules' 🛑 list), so the dev box going down takes it. ⚠ **(2) A REAL DEGRADATION WENT UNREAD FOR FOUR DAYS.** `status` was `degraded` on **4 of 2886** samples, escalating **3 → 4 → 5 → 6 consecutive poll failures**, `last: ConnectError`, then recovering. Inbound was genuinely DOWN and `/healthz` said so — **hard rule #5 working exactly as designed, and nobody looking.** The reason string is the one that cannot distinguish revoked key / deleted subscription / wrong name / quota, so **which of those it was is now unrecoverable**; that is a finding about *retention of evidence*, not about the incident. ⚠ **(3) `events_seen` IS `0 → 0` ACROSS THE WHOLE WINDOW**, and so are `suppressed_opt_out` and `suppressed_not_authorized`. `CLAUDE.md` predicts `suppressed_opt_out` **moves** on Ai Trader traffic now the app is in four spaces. Over 24 h it did not move at all. **Two readings and this row refuses to pick one without evidence:** nothing was posted in any of the four spaces for a day, or **the inbound path does not work end-to-end in production**. ⚠ **Finding (2) is NOT evidence for either reading and must not be read as leaning toward the alarming one** — a few minutes of `ConnectError` does not mechanically explain 24 h of zero events, and is equally compatible with *"quiet, and also had a blip"*. The two are recorded together because they are the same window, not because one implies the other. ⚠ **Not a ledger claim in either direction** — `events_seen: 0` is exactly what CG-55 and CG-59 both recorded, and a zero proves neither delivery nor its absence. ~~⚠ **THE UPTIME EVIDENCE IS A SEPARATE THING AND IT IS STILL LIVE:** container start `2026-08-05T16:34:10Z`, `RestartCount: 0` — **~5 days, longer than the soak ever asked for**, and the strongest single input to retiring the `SubscriberLoop` long-run ⚠ flag. **CG-80's rebuild spends it permanently.** One `docker inspect` before that rebuild preserves it; **this row's first task is that capture**, and it is the only part with an ordering constraint against CG-80.~~ ⛔ **STRUCK 2026-08-11 — see the correction at the head of this cell.** Kept struck rather than deleted because it is the clearest specimen of the failure it warns about: a fact re-asserted in the **present tense**, decorated with a **✅ STILL LIVE** in the banner, by documents on their own schedule, **while nothing was measuring the thing described.** The `~5 days` was never observed; it was **arithmetic on a start timestamp**, and the four days it counted are exactly the four days nobody looked. Retiring the flag itself stays a hard-rule-#3 decision for the user — this row produces evidence, not a clear, ⚠ **and it now produces materially less of it than it promised.** |
| **CG-83** · `/v1/messages` prints **any** exception into the delivery log — CG-29's rule has a hole its own MCP sibling does not | 📋 **queued** · ⚠ **latent, not live — severity stated precisely below** | **Found by CG-80's Builder, filed 2026-08-10 by the Coordinator** — ⚠ **because CG-80's row promised it was "filed below" and it was NOT; the only thing filed there was a different finding (the `AuthError`/CG-73 one). A security note buried in a COMPLETED row's prose is a note nobody will ever work.** `service.py:414` does `log.record(..., "failed", str(exc)[:200])` and `:415` does `detail=str(exc)`, both under a **bare `except Exception`** — so **any** exception type is printed in full, which is exactly what CG-29's allowlist (`describe_exception`, marked classes only) exists to forbid. CG-80's **new** write site in `mcp.py` uses `describe_exception` correctly; the old sibling was deliberately left alone because spec §15 forbids touching that route. ⚠ **SEVERITY, measured rather than assumed — this is LATENT, not leaking today.** The mechanism is real: `str()` of an `httpx.HTTPStatusError` carries the full URL, and a webhook URL embeds `key` and `token` (verified 2026-08-10 by constructing one — both appeared in full inside 200 chars). **But every adapter today wraps its httpx errors** into marked classes with repo-authored messages carrying only `type(exc).__name__` (`webhook.py:140`, `chat_api.py:147`, `:213`), and none calls `raise_for_status()`. **So no current code path delivers a credential-bearing exception to that line** — CG-23's fix is holding, and the leak CG-80's Builder "measured" was an **injected** `_LeakyError` in a test, not production traffic. ⚠ **What makes it worth a row anyway:** the guard is one `except`-clause away from being load-bearing, it protects a file **persisted to disk** (`deliveries/`, which ADR-0002 **D7** exempts from all pruning — so a leak there is permanent), the same `str(exc)` also reaches an **HTTP response body**, and the next adapter that forgets to wrap re-opens the exact hole CG-23 closed after a real 403 put a `key` and `token` into three artifacts. ⚠ **Do not fold this into CG-73** — that row is about `AuthError` and the 401 path; this is a different site, a different exception surface and a different blast radius, and CG-80's Builder correctly filed the `AuthError` half there rather than here. |
...
| **CG-88** · `allow_inbound` defaulted to **`True`** — omitting the key was "on", not "off" | ✅ **shipped 2026-08-31** · [PR #77](https://github.com/mmackelprang/chat-gateway/pull/77) | **Owner ruling 2026-08-31: fix the default.** `registry.py`'s `App.allow_inbound` defaulted to `True`, so an app that never mentioned inbound HAD it — **which is why CG-61 exists**: `aiteam-harness` ran open for its whole life for exactly this reason. Hard rule #6 says inbound crosses *"only by that consumer's explicit registry opt-in"*; with a permissive default that sentence was **aspirational**. ⛔ **THE REASON IT IS MORE THAN TIDINESS:** `docs/consumers/aitrader.md` §8 — *"No inbound path — the guarantee, restated on its real basis"* — promises a **real-money tenant** no inbound path, and all four of its enforcement points read `app.allow_inbound`. That guarantee was held by **one YAML line in a file with THREE copies, only one of them in git**, whose install step overwrites the box's copy with a checkout's. Dropped, reformatted away, or missing from a copy, **the line's absence INVERTED the guarantee in silence** — nothing logged it, nothing counted it, `/healthz` read `ok`. ✅ **Default is now DENY.** ⚠ **`required, no default` was the stronger shape and was DECLINED, with the trade stated rather than taken blind:** a registry omitting the key would refuse to load, and **the NAS copy cannot be read from any checkout** (0d) — ⚠ **ONE copy, not two: corrected in pre-merge review, because the dev-box file is gitignored (absent from a fresh clone) yet present, readable and READ on this machine, and overstating the unknown by 100% weakens the very record that justified declining the stronger shape** — so the reliance is **REPORTED** instead: `Registry.inbound_defaulted` names every app that left the posture to the loader, on `/healthz` (⚠ **never an input to `status`** — the gateway is DENYING, and degrading on a guarantee working is the verdict `suppressed_opt_out` and `files_deleted` already got) and as a **stderr** boot warning (stdout is `check`'s JSON). ⚠ **REPORTED IS NOT ENFORCED and no document may say otherwise.** ⛔ **A SECOND INVERSION IN THE SAME FIELD, closed with it:** `bool("false")` is `True`, so `allow_inbound: "false"` — a *quoted* boolean, which is what a YAML formatter or a templating pass produces without being asked — read as **true** under `bool(spec.get(...))`: the entry granted precisely what it appeared to refuse. That is the *"reformatted away"* half of the same defect, and a non-boolean is now a load-time `RegistryError` — `_require_id_str`'s existing treatment of YAML coercion traps in that very file. ⛔ **IT IS A NEW WAY TO TAKE THE GATEWAY DOWN, and the first draft of this row got the consequence wrong in the reassuring direction** — it cited *"PR 4's R1-B"* for an unloadable registry being a supported steady state, which is **another repository's ruling about a different file**. Here `load_registry` raising means `main` prints `config error:` and exits **2**: the gateway does not start, and there is no degraded mode. Caught in this row's own self-review. ⚠ **Taken anyway, with the alternative named:** deny-and-warn on a non-boolean carries no outage risk and was declined because it silently discards what an author wrote (`allow_inbound: "true"` would become a refusal reported only in a log line). What bounds the risk is that a non-boolean **cannot arrive by omission** — it has to be typed, both readable copies use plain booleans, and the message names the app and the fix. ⛔⛔ **THE LOUD OUTAGE PATH, FOUND IN SELF-REVIEW AND MEASURED AGAINST BOTH LOADERS — and it is NOT the quoted boolean.** ⚠ **It was written as *"THE ONE"* until pre-merge review named a SECOND, quieter one: an app that relied on the default and merely POLLS `/v1/inbox` causes no refusal at all — it boots and starts getting 403s, and a space-owning one silently increments `suppressed_opt_out` instead of delivering. That is the design working; the point is that the loud path is the one this row named and the quiet one is the one that would take longer to find.** An app with a **`callback_url` and no `allow_inbound`** LOADS on `main` (inbound on, by the old default) and **REFUSES on this branch**, because `callback_url requires allow_inbound: true` now fires — so the gateway does not start. ⚠ **Unlike a quoted boolean this is producible by OMISSION**, which is the accidental case, and the NAS copy cannot be read to rule it out (0d). ✅ **Today's readable copies are both safe** — the dev-box registry and the committed example each write `allow_inbound: true` beside `job-hunter`'s callback — **but that is one deployment, not a guarantee.** ✅ **Mitigated where an operator will actually meet it:** the refusal now appends a hint naming CG-88 and the one-line fix, **appended rather than woven in**, because `aitrader.md` §8 quotes the unhinted message verbatim and that tenant WRITES its `false` — so the hint cannot fire there and the published quotation stays byte-exact, pinned by a test that reads the doc. ⚠⚠ **TEN EXISTING TESTS WENT RED THE MOMENT THE DEFAULT FLIPPED** — six in `test_adapters.py`, four in `test_service.py` — **every one an *inbound* assertion resting on a default nobody had chosen.** That is the honest measure of what the old value was holding up; both fixtures now write the key. ⚠⚠ **AND ONE NEW CONTROL WAS GREEN ON ITS FIRST MUTATION:** flipping the **dataclass** default back to `True` left the whole suite passing — **528 at that moment**, which is the suite BEFORE this PR's later controls landed and is why it does not match the **533** at the end of this cell. ⚠ **This clause has itself now been wrong twice — it read `531` until the review findings added two more tests — which is the strongest available argument for not quoting a moving number twice in one paragraph at all.** Re-run against the final tree, M2 is RED, because `load_registry` passes the field explicitly on every path — **two sites, one bound** (0h). **ELEVEN single-site mutations, all RED**, each applied alone with `__pycache__` cleared on both the mutated and the reverted run and the file asserted byte-identical after every revert. ⛔ **AND THE HARNESS LEAKED A MUTATION INTO THE SHARED WORKING TREE — twice in one PR, by two different agents, and the rule is the same both times: A MUTATION HARNESS MUST RUN IN AN ISOLATED COPY AND RESTORE ON ANY EXIT, INCLUDING A KILL.** The pre-merge reviewer disclosed one (restored the moment it noticed); the Builder's own harness was then **killed by a 10-minute timeout mid-run** and left `sorted(...)` → `list(...)` in `registry.py`, because the revert ran after the subprocess rather than in a `finally`. ⚠ **A suite run BEFORE the leak proves nothing about the tree AFTER it**, and the leak was found by counting fragments before the next batch rather than by any control. Both batches were re-run in a `tar`-copied tree that is diffed against the working tree afterwards, and the restore is now in a `finally` with a byte-identity assertion — including the two that guard the appended hint in BOTH directions: never firing, and firing always (which would corrupt the message `aitrader.md` quotes verbatim). ⚠ **ONE FIELD, NOT THE CLASS:** the census found exactly one other fail-open default in `registry.py` — `allowed_users` is *empty = anyone* — **filed as CG-89, not fixed**, because unlike this one that default was CHOSEN and documented at the field. ⚠ **Merged is not in effect** (CG-61's lesson, CG-80's repeat): the default binds at **load**, so a running gateway keeps the posture it booted with and the NAS gets this at its next redeploy. ⚠ **And it cannot read the NAS copy** — it protects an entry that says nothing; it cannot tell you what the box's file says. **511 → 533 tests.** |
| **CG-89** · `allowed_users: []` means *anyone* — the one other fail-open default in `registry.py` | 📋 **queued** · **needs a user decision — it changes who may act in a two-way tenant's space** | **Filed 2026-08-31 by CG-88's Builder, from the census that row was asked for, and deliberately not folded into it.** `App.allowed_users` defaults to `[]` and `adapters/pubsub.py`'s check is `if app.allowed_users and sender not in app.allowed_users:` — so an **empty list authorizes every sender**, and a two-way tenant that omits the key gets no per-user gate at all. ⚠ **NOT the same defect as CG-88 and that is why it is a row rather than a commit.** `allow_inbound: true` was a default **nobody chose**; *empty = no restriction* is **written at the field** (`registry.py`'s own comment) and is a legitimate configuration — a space where everyone present is trusted has no other way to say so, and flipping it to deny-all would need a wildcard nobody has designed. ⚠ **Its blast radius is a SILENT INBOUND OUTAGE, not a leak**: a tenant that omitted the key would begin refusing every human in-thread, which is the opposite failure from CG-88's and lands on a live user rather than on a log. ✅ **It is strictly narrower than it was**, because it is now reachable only behind an explicit `allow_inbound: true`; **narrower is not closed.** ⚠ **Today it is inert** — `job-hunter` is the only two-way tenant and both the committed example and the dev-box registry write the list — but *inert* is a property of one deployment and the census could not read the NAS copy. **Options, none chosen:** default-deny with a documented wildcard (`allowed_users: ["*"]`); require the key whenever `allow_inbound: true`; report the reliance the way CG-88 reports its own, and change nothing; or leave it and record the choice. ⚠ **The census's own blind spot, stated: it read `registry.py` only, and it reads DECLARED defaults — `[]` looks neutral, and only `pubsub.py`'s `if` makes it permissive.** A default whose permissive meaning lives entirely in a distant consumer would not have been found. |
...
**both**, then `grep` for anything else asserting what you just changed. Recorded
here, at the head of the thing it governs, 2026-08-11, after the mistake was made
twice in one day.

⚠ **~~FOUR~~ ~~SIX~~ EIGHT ROWS BREAK THAT PATTERN AND HAVE NO DETAIL SECTION:
CG-82, CG-83, CG-84, CG-85 — and CG-86 and CG-87, added 2026-08-31, and CG-88
and CG-89, added later the same day.** Their table cells **are** their canonical write-up; there is nothing
...
CG-84, CG-85, CG-86, CG-87, CG-88 and CG-89 follow it rather than reopening the question. ⚠ **The last two were added to the headline count and NOT to this sentence, and that was caught in pre-merge review — the second list in the paragraph about keeping two copies in sync, out of sync with the first.**
...
seventh owes the same edit. ⚠ **CG-88 and CG-89 were the seventh and eighth,
and paid it in their own commit — twice in one day, by two different rows,
which is the strongest evidence available that the instruction works and the
shape does not.** ⚠ **Whether to keep
...
see its row). CG-54 and CG-75 are done.~~ ⚠ **Nothing gates CG-55 any more — it
DEPLOYED and MERGED on 2026-08-05** (`4ddd6f5`). Corrected 2026-08-05 (CG-79);
the struck sentence was **stale on all three of its claims at once** — CG-53 is
`✅ done (#65)`, CG-61's operator action landed 2026-08-03, and the row they
gated has shipped. **A sentence that enumerates gates is the worst shape to leave
standing**, because each item ages independently and the sentence reads as
current until *every* one of them is checked. The reasoning and the accepted
residual live in **CG-55's row** — one home, not three.
...
✅ **That is now history rather than intent — the deploy landed 2026-08-05 and
the clock has been running since** (noted 2026-08-05, CG-79). The sequencing
argument is left in the future tense it was written in, because it is a record of
why the order was chosen; what follows from it — that **CG-59 is unblocked and
accruing value every day it waits** — is recorded on CG-59's own row.
...
### CG-61 · Close `aiteam-harness`'s inbound path  ✅ **DONE** — merged ([#50](https://github.com/mmackelprang/chat-gateway/pull/50), `dced002`) **and the operator action complete (2026-08-03)**

> ⚠ **This heading read `⏸ OPERATOR ACTION OUTSTANDING · BEFORE CG-55` until
> 2026-08-05.** Both halves are spent: the operator edit landed **2026-08-03**,
> and CG-55 — the row it was sequenced before — deployed and merged **2026-08-05**.
> Corrected by CG-79, and the heading is the site that matters most, because a
> heading is what a reader takes on trust without opening the row.

| | |
|---|---|
| **Decision** | **user, 2026-07-31 (D1)** — from Planner's finding while correcting the four-spaces premise |
| **Depends on** | nothing. ~~**CG-55 depends on THIS** being in the live registry~~ ✅ **satisfied** — the live edit landed 2026-08-03, CG-55's pre-flight confirmed it 2026-08-05 |
...
✅ **The edit landed 2026-08-03 and the pre-flight is what proved it** (recorded
2026-08-05, CG-79). `docs/deploy/nas.md:609` logs the PASS on the box:
`{aiteam-harness: False, job-hunter: True, aitrader: False}` from the real
`load_registry` against the gitignored file, run as a fail-closed script rather
than a memory test. **The paragraph above is the design that worked** — it
assumed nobody would remember, and nobody did: this file went on calling the
action outstanding for two more days while the deployed gateway was already
running with the path closed. Left standing in the present tense it was written
in, because it is a statement about what CG-55 must carry, not about a date.
...
- **The pre-state was re-measured, not copied from this row.** The real
  `load_registry` against the real live file returned `aiteam-harness
  allow_inbound=True` **on 2026-07-31**, and the key was **absent** — so the
  default was what granted inbound, exactly as D1 argues. This row's ⚠ paragraph
  above was right. *(Tense fixed 2026-08-05 by CG-79: this said* ***"returns …
  today"***, *and "today" in a merge note is a date that stops being one the
  moment the note is filed. The post-state is measured above.)*
- ⚠ **Merging is not finishing.** The PR changes `registry.example.yaml`; the
  ...
  comment, each of which now names the other so neither is missed.
  ✅ **The edit landed 2026-08-03; both "not yet"s were corrected 2026-08-05 by
  CG-79, and the cross-reference is why both moved together.** That pairing —
  *"whoever makes it owns both"* — is the one control in this whole row that did
  its job across a two-day gap and a different Builder. ⚠ **It also has a
  measured limit worth recording:** it named the two sites that would go stale
  and nothing named *this row's own status cells*, which is why five of them sat
  false while the two it did name were fixed in a single sweep. A cross-reference
  is a pointer, not an inventory — which is CG-69's entire thesis. The plan's
...
### CG-53 · Deployment artifacts and the secret-safety proof (**no deploy**)  ✅ **DONE (#65, 2026-08-03)** — gate released by the user, merged on green gates
...
| **Merge gate** | ✅ **RELEASED by the user 2026-08-03**, on the four things §4.1 says releasing it approves. Was: ⏸ the gate CG-23, CG-33, CG-34 and CG-51 carried |
...
#### ✅ Shipped as #65 — what landed, and the two things it hands back

Plan Part A, tasks A1–A7. Suite **359 → 382**. No ⚠ verification-ledger flag
cleared, added or reworded (`git diff main -- src/ | grep -c` = **0**) — this row
exercises nothing against Google.

**The premise was demonstrated, not asserted.** Through the homelab repo's
**real** `redact_json.py` against a realistically-shaped captured payload (shape
copied from the real `nas/compose/homepage.config.json`, values synthetic):
three `CHAT_GATEWAY_API_KEY__*` and two `GOOGLE_CHAT_WEBHOOK_URL__*` survived
**verbatim** and the scan gate **exited 0**, while a `POSTGRES_PASSWORD` control
in the same payload was redacted. The CG-53-form capture — paths only — carries
**zero** credential-shaped substrings. `SECRET_KEY_SUFFIXES` re-counted: **18**.

⚠ **Two items handed back rather than decided here:**

1. **`service.py`'s `/healthz` reason text for `CHAT_GATEWAY_INTERACTION_ROUTING_TARGET`
   is add-ons-era and tells an operator to set the Pub/Sub **TOPIC path**.**
   Production has been **classic** since 2026-07-29, where the value is any
   constant (ADR-0001 D3), and following the machine's own advice costs the
   native action-identity slot. Same class of leftover CG-21 cleaned out of four
   documents and missed here. **Not fixed** — it is in `src/`, outside this row's
   lane, and it is a Planner call. Filed here rather than routed around.
2. **The runbook's §1 publishes a service inventory and a 23-port list for a
   private home network, in a public repo.** That content is **plan-mandated**
   (Part A §A6 §1, verbatim) and is within what spec §4.1 item 3 describes as
   *"a real description of a real box's shape"* — which the user was shown when
   releasing the gate. It contains **no** identity literal (no host, user, IP,
   key path, email, space id or webhook URL — scanned adversarially, and CG-26's
   guard passes). Recorded as an accepted residual to revisit, **not** trimmed
   unilaterally, because trimming approved plan content is re-planning.
...
this a no-op in **every** existing test — this row said `202`, then `345`, and
CG-76 made it **359** hours later; **the count belongs to the suite, not to this
row**, and the two corrections in one day are the argument for saying so rather
than an exception to it); **no new dependency** (~20 lines);
...
- ⚠ **The NAS is NOT "backup target only", and this is NOT a role change.** That
  ...
  claude-mem's Postgres. The real question is what **one more stack** costs, and
  it is answerable: *(said "a **tenth** stack" until 2026-08-05 — with 10 already
  running the gateway is the **eleventh**, confirmed by the live `app.query`'s 11
  apps. Wrong when written, not gone stale; CG-79, and `docs/deploy/nas.md` §1
  carries the account of how a heading and its own body disagreed for five days)* **20.8 GB RAM available of 31.9 GB, load 0.29 on 16 cores,
...
| 4 | **§7 said "Run `capture.sh`" — that contradicts this plan's own standing rules**, where it is a 🛑 (it rewrites every stack's file — ten then, **eleven** now; CG-79). Part C already had it right. Now: request the run, read the output | process |
...
| 9 | **`202` → 345 → 359** (CG-76 landed mid-refresh), and A7 now says take it from the suite — it has been stale **twice in one day** | stale |
...
| 6 | **#66 / CG-55 merging** — and CG-61's operator edit two days earlier | *"PR open and HELD for the user's review; **not merged**"* (and *"operator action outstanding"*) — stale within hours, in **the first file a Builder reads**, across **40+ passages in 7 files** | `BUILDER_QUEUE.md` banner + siblings everywhere; found by the user, not by a diff |
...
never been deployed (ADR-0002 §8), so no such file exists in production~~ — ⚠ **that
premise expired 2026-08-05 when the gateway deployed, and the conclusion survives
on measured evidence instead of on the premise** (CG-79). CG-55 ran a
historical-mode sweep on the box: the gateway's own writers created **every** file
`0600` and the `! -perm 600` probe printed nothing (`docs/deploy/nas.md` §10 Steps).
So there is still no such file in production — now because it was **looked for**
rather than because nothing was running, which is the stronger of the two reasons
and the one this row should have wanted all along. **The severity is unchanged;
what changed is that it now rests on a measurement.** And the
...
### CG-79 · Reconcile CG-55's recorded status across the repo  ✅ shipped 2026-08-05 · [PR #68](https://github.com/mmackelprang/chat-gateway/pull/68)

| | |
|---|---|
| **Origin** | user-flagged 2026-08-05, after **CG-78's Builder** spotted the queue banner and **correctly declined to edit another row**. That restraint is the reason this row exists rather than the reason it was needed |
| **Depends on** | nothing |
| **Touches** | `docs/BUILDER_QUEUE.md`, `CLAUDE.md`, `docs/deploy/nas.md`, `docs/google-cloud-setup.md`, `docs/consumers/aitrader.md`, `docs/consumers/jobhunt.md`, `src/chat_gateway/adapters/pubsub.py` (one comment) |
| **Merge gate** | no — docs plus one comment |
| **Falsifies** | six live claims (below). **Changes no behaviour and moves no ⚠ flag** |

**The row was handed one sentence and corrected more than forty passages across seven files.** The banner said
CG-55's PR was *"open and HELD for the user's review; not merged"*. It had merged
hours earlier the same day. **That was the least interesting of the six things
that turned out to be false**, and the instruction that made the row worth doing
was *do not fix only the sentence I quoted*.

#### The six falsified facts, and what each one cost

| # | The claim | What is true | Where it was wrong |
|---|---|---|---|
| 1 | CG-55's PR is *"open and HELD … not merged"* | **[#66](https://github.com/mmackelprang/chat-gateway/pull/66) merged 2026-08-05 as `4ddd6f5`** (`gh pr view`) | banner, summary-table cell, section heading, merge-gate row |
| 2 | CG-61's live-registry operator action is *"outstanding"* | **done 2026-08-03**, mtime `2026-08-03T20:34:06Z`; `aiteam-harness allow_inbound=False` through the real `load_registry`, key **explicitly present** at line 38 | 7 sites incl. one labelled *"the currency pointer"* and one certified *"NOT stale"* |
| 3 | `capture.sh` *"not run — outstanding"* | **run by the user 2026-08-05**, output verified clean | banner, table cell, the row's *"what is left OPEN"* table, the success gate, `nas.md` ×2 |
| 4 | `iac/chat-gateway-sa.json` is *"STILL PRESENT"* | **deleted 2026-08-05** | 7 sites across 4 files (5 outside this file, enumerated in CG-55's row) |
| 5 | The gateway *"has never been deployed"* | deployed and serving | CG-55's own decision section, CG-70's severity rationale |
| 6 | The gateway is *"the **tenth** stack"* | **the eleventh** — 10 pre-existing + 1; the live `app.query` lists **11 apps** | `nas.md` §1 heading, 2 queue sites, 3 *"all ten stacks"* rules |

#### ⚠ Three of these were already false when CG-55 read them — and that is the finding

**CG-61's operator edit landed at 20:34Z on 2026-08-03. The measurement this file
kept quoting as current was taken at 19:15:56Z the same day — 78 minutes
earlier.** So when CG-55's Builder came to deploy on 2026-08-05, the queue told it
a hard prerequisite was unsatisfied, and it was wrong.

**Nothing shipped broken, and the reason is a single sentence somebody wrote into
the CG-61 row: *"Re-measure rather than trusting this line."*** CG-55 carried a
**fail-closed pre-flight** instead of a memory check, ran it on the box, and got
`{aiteam-harness: False, job-hunter: True, aitrader: False}`
(`docs/deploy/nas.md:609`). **The correct fact was therefore written down in this
repo, in a different file, on 2026-08-05 — while this file went on calling it
outstanding.** That is not a stale-docs problem. That is two homes for a moving
fact, which is the failure this project names more often than any other.

⚠ **The two sentences that aged worst were the two that certified themselves.**
One called itself *"the currency pointer"*; another declared a neighbouring claim
*"NOT stale"* and was right on the day. **A sentence that asserts its own currency
is read as verified rather than as merely current**, so it is examined less, not
more. Both are struck in place with this reasoning attached, because the shape
will recur and the wording is the only warning available.

#### Fact 6 is a different class from the other five — and worse

The other five are **CG-69 category (a)**: live claims falsified by a later
change. **The stack count is category (b) — wrong when written.** 10 stacks were
already measured in `nas.md` §1's own body when the same section's **heading**
called the gateway *"the tenth stack"*. The right number was three lines from the
wrong one, for five days.

**It survived because of where it sat.** A heading reads as a title and a
paragraph reads as data; nobody compares them. It then **propagated by
quotation** into a briefing and into a sibling agent's task, and was caught only
because that agent independently counted the live `app.query` and recorded the
discrepancy rather than deferring to what it had been told. ⚠ **CG-69's spec §8
says plainly that no guard in this repo can reach category (b)** — this row is
evidence for that claim, not a counterexample to it.

#### The dead-key warnings are REWORDED, not dropped — on CG-55's own instruction

CG-55's row set the condition in advance: *"the deletion is the user's; when it
lands, both need **rewording, not dropping** — a warning that simply vanishes is
indistinguishable from one nobody thought about."* It landed. All five sites were
reworded, and the rewording changes what they warn **about**:

**from a path — `iac/chat-gateway-sa.json` — to a shape: any
`chat-gateway-sa*.json` that is not `chat-gateway-sa-gw.json`.** The file is gone
and the hazard is not: old checkouts, backups and clones still hold the dead key,
and **`iac/gcloud-setup.sh` and its `.ps1` sibling still default `KEY_FILE` to
that exact filename**, so re-running setup writes it again. A warning keyed on a
path this repo has deleted protects nobody; one keyed on the shape survives the
deletion. Every site now also says **confirm by the key's own `project_id`, not
by its name** — which is what the deploy actually did (`nas.md` §10 deviation 7).

⚠ **`adapters/chat_api.py`'s docstring mentions the dead key and was deliberately
NOT touched.** It sits inside a **⚠ flag CLEARED** entry, as a dated observation
about which key minted a real token. It asserts nothing about the file existing,
and editing prose inside a ledger entry is flag movement by another name.

#### ⚠ Not fixed here — routed to Planner, with the list

**`docs/superpowers/` specs and plans carry the same stale claims, and this row
did not edit them.** A Builder does not rewrite a Planner artifact; CG-53's row
set that precedent twice in one section. **But CG-69 evidence instance 5 is
exactly a case where a Builder stayed correctly in its lane and a false claim
shipped anyway** — so the finding is routed rather than merely declined, with
file and line, which is the whole point of that control:

| File | Lines | Stale on |
|---|---|---|
| `superpowers/specs/2026-07-31-production-readiness-arc-design.md` | 11; 179, 583, 588 | *"It has never been deployed"*; *"tenth stack"* ×3 |
| `superpowers/specs/2026-07-31-production-readiness-arc-design.md` | 425, 429; 759; 1001, 1026, 1101, 1496 | CG-55 *"user-executed"* + CG-59 *"strictly after a deploy"*; dead key; *"ten stacks"* ×4 |
| `superpowers/plans/2026-07-31-production-readiness-arc.md` | 42, 432, 478, 566, 720, 802, 1716, 1761, 1808, 1810 | *"all ten stacks"* ×4; *"tenth stack"*; dead key ×3; *"never been deployed"*; *"deployed gateway runs `allow_inbound: true`"* |
| `superpowers/plans/2026-08-03-published-promise-inventory.md` | 518 | *"nothing is deployed yet"* |
| `superpowers/specs/2026-08-03-delivery-write-path-robustness-design.md` | 567 | *"has never run on a box with a real disk that can fill"* |
| `superpowers/plans/2026-07-29-live-verification-followups.md` | 105 | rotation recipe keyed on the deleted key's filename |

**Every line number above was re-checked with `grep` in this repo before being
published** — not carried from the sweep that found them, because a routing list
whose coordinates are wrong is worse than no list. Two of the sweep's rows did not
survive that check and are **deliberately absent**: spec `:838` (*"all ten"*
counts `nas/services/*.md` **docs**, a different set of ten and still correct) and
spec `:1418` (*"user-executed"* there is CG-59's observation half, which really is).

⚠ **`plans/2026-07-31-production-readiness-arc.md:1761` is the one to read first.**
It says a *"deployed gateway runs `allow_inbound: true` and the first drain
writes"* — a **security-shaped** claim about a live deployment, and it is now
false in the safe direction. A Planner reconciling this list should start there.

#### ⚠ Three numbers this row got wrong in its own draft — and it is a row about wrong numbers

**Recorded rather than quietly fixed, because a row whose entire subject is
inaccurate bookkeeping does not get to be silently inaccurate.** Two were caught
by **pre-merge review**, one by the user; **none by the author.**

| # | Draft said | Truth | How it was caught |
|---|---|---|---|
| 1 | the CG-61 edit landed *"**79** minutes after"* the 19:15:56Z measurement, in **five** places | `20:34:06 − 19:15:56` = **1h 18m 10s** → **78 minutes** | reviewer subtracted the two timestamps **printed in the same sentence** |
| 2 | routing table: plan carries *"all ten stacks"* **×5** | **×4** (lines 42, 720, 1716, 1810) — the row's own tallies summed to 11 against 10 listed line numbers | reviewer counted the row's tallies against its own coordinates |
| 3 | the captured config has **four** env vars | **six** — plus `GATEWAY_ENABLE_PUBSUB` and `TZ` | user; the grep pattern `CHAT_GATEWAY_[A-Z_]+\|GOOGLE_[A-Z_]+` **structurally could not match either** |

⚠ **#1 is the sharpest, and it indicts the format rather than the arithmetic.**
The wrong figure sat **immediately beside both timestamps that disprove it**, in
the same sentence, five times over — the identical shape as fact 6, where
`nas.md` §1's heading contradicted its own body three lines down. **Twice in one
PR, a derived number and the data it derives from sat side by side and disagreed,
and the human-readable derived number is the one that got trusted.** That is not
carelessness; it is what prose does. It is also the best argument in this file for
CG-69's control being *executable* rather than editorial.

⚠ **A fourth number was DELETED rather than corrected: the count of sites this row
touched.** The draft said *"21 sites"* in five places. It is **more than forty**,
and rather than assert a precise figure nobody can re-derive, the claim is now a
**checkable floor**. Having got three counts wrong already, publishing a fourth
precise one would have been the joke telling itself.

#### The env-var count, and why it was deleted rather than fixed

The first draft of the capture summary said *"717 bytes, **four** env vars"*. It
is **six** — the four paths plus `GATEWAY_ENABLE_PUBSUB` and `TZ` — and the four
came from a grep pattern (`CHAT_GATEWAY_[A-Z_]+|GOOGLE_[A-Z_]+`) that
**structurally could not match either extra**. Both extras are non-secret, so the
*"zero secret values"* verdict never moved.

**The fix was not to correct the number but to delete it.** That count lives in
the **homelab** repo and has no business having a second home in a public one —
which is the same lesson as the test count, arrived at by walking into it inside
a row written to catch exactly this.

#### What this row does NOT do

- **It does not re-verify the deploy.** The five facts, the seven deviations and
  the counters keep their one home in `docs/deploy/nas.md` §10 and are **not**
  summarized here.
- **It moves no ⚠ verification-ledger flag.**
  `git diff main -- src/ | grep -c "LIVE-UNVERIFIED\|SHAPE-VERIFIED"` → **0**.
  A live deploy plus a merged PR is a tempting moment to retire the
  `SubscriberLoop` long-run row; it clears by **soak (CG-59)**, not by deploying,
  and not by a bookkeeping PR.
- **It closes nothing that is actually open.** The homelab §9 artifacts and
  CG-59's soak are still outstanding and are recorded as such in three places.
- **It does not reconcile the Planner artifacts above** — it lists them.

⚠ **Logged as CG-69 evidence instance 6.** First instance whose falsified claim is
about **this file's own bookkeeping** rather than about code — and the first found
by a human reading the file, not by a sweep, a test or a reviewer.
...
### CG-55 · First NAS deploy and live smoke  ✅ **DEPLOYED 2026-08-05** · **[PR #66](https://github.com/mmackelprang/chat-gateway/pull/66) MERGED** as `4ddd6f5` · **BUILDER-EXECUTED over SSH**
...
> just warned about:**
>
> **⚠ Status of this table reconciled 2026-08-05 by CG-79. Two of its four rows
> closed, and the third and fourth are the only work this row still owns.**
>
> | Open | Owner | Evidence |
> |---|---|---|
> | ~~`capture.sh` not run — 🛑 for Builder, **requested and outstanding**~~ ✅ **CLOSED — run by the user 2026-08-05, output verified clean** | was: whoever holds the homelab working tree | §10 fact 5 · the captured JSON is the **homelab** repo's artifact, named below and **not reproduced** |
> | every §9 homelab-side artifact (`nas/services/`, `SECRETS.template.md`, `restore-*.sh`, `DASHBOARDS.md`, Homepage tile) — ⏸ **STILL OPEN**, a PR is being prepared in that repo | homelab repo, deploy-then-document | §10 *What this run did NOT do* |
> | the tailnet ACL (D2) **not applied at deploy time** — ⏸ **still not applied**, and still deferred | deferred by the user; re-priced by the subnet router | §10 Steps table |
> | what the subscription actually carries, and the `SubscriberLoop` long-run ⚠ flag — ⏸ **STILL OPEN** | **CG-59** — it needs a clock, not a smoke test; the clock started 2026-08-05 | §10 *C1a* |
>
> **The capture, recorded as a fact and not as an artifact.** `capture.sh` was
> run by the user; its output is `nas/compose/chat-gateway.config.json`, and it
> was scanned for the credential shapes this project's names defeat (`cgw_`,
> `chat.googleapis.com`, `token=`, `key=`, `private_key`, PEM headers) — **zero
> hits**. Every env var it carries is a **path** or a non-secret flag. ⚠ **That
> file lives in the homelab repo, not this one, and its counts are therefore NOT
> restated here.** The verdict is recorded because this row's success gate
> depended on it; **the contents and their tallies are not**, because a second
> copy of a captured config in a *public* repo is precisely the outcome the gate
> exists to prevent — and because a count copied out of another repo is this
> project's oldest recorded failure mode. ⚠ **This paragraph proved that in the
> writing.** Its first draft said *"**717 bytes**, **four** env vars"*; the count
> was **six** (the four paths plus `GATEWAY_ENABLE_PUBSUB` and `TZ`), and the
> four came from a grep whose pattern — `CHAT_GATEWAY_[A-Z_]+|GOOGLE_[A-Z_]+` —
> **structurally could not match either extra**. Caught before merge. Both extras
> are non-secret, so the *"zero secret values"* verdict never moved; what moved
> was a number that had no business being in this file at all, which is why it is
> now absent rather than corrected. ⚠ **`clean. safe to commit.` was not the
> evidence** — this row said in advance that the script's suffix rule cannot see
> these shapes, so the assurance is worth nothing; the explicit shape scan is what
> answered it.
>
> ⚠ **`iac/chat-gateway-sa.json` — the dead key — is now DELETED, and this row's
> instruction has been carried out** (2026-08-05, CG-79). ~~It is STILL
> PRESENT, so the dead-key warnings in `CLAUDE.md` and `docs/deploy/nas.md` §8
> are **still current** and were left standing deliberately.~~ **The condition
> that paragraph named came true.** All five sites — `CLAUDE.md`,
> `docs/deploy/nas.md` §8, `docs/deploy/nas.md` §10 deviation 7,
> `docs/google-cloud-setup.md` twice — were **reworded, not dropped**, because
> *"a warning that simply vanishes is indistinguishable from one nobody thought
> about"* and that reasoning is the durable half of this note. What each of them
> now says is the thing that outlives the file: **the key that authenticates is
> `chat-gateway-sa-gw.json`, and any *other* `chat-gateway-sa*.json` a reader
> finds — in an old checkout, a backup, a stale clone — belongs to the deleted
> `chat-gateway-prod` and will not authenticate.** ⚠ **Deliberately NOT touched:
> `adapters/chat_api.py`'s docstring**, which mentions the dead key inside a
> **⚠ flag CLEARED** entry. That is a dated observation about which key minted a
> real token, it asserts nothing about the file existing, and editing prose
> inside a ledger entry is flag movement by another name.

| | |
|---|---|
| **Depends on** | **CG-53** (artifacts), **CG-54** (durability), **CG-75** (the storming write path), **CG-76** (⚠ **added 2026-08-03 — the dependency was NOT recorded here.** The user's 2026-08-02 pre-deploy-blocker decision existed; measured the same day, `grep -n "CG-76"` named it nowhere under either CG-55 entry. aitrader's whole gateway relationship is the dead-man switch, and that switch has **six** measured ways to drop an alert while `/healthz` answers `ok` — three of them needing no fault at all. A first deploy should not ship that), **CG-61** (in the LIVE registry — D1; ✅ **satisfied — the operator edit landed 2026-08-03 and the pre-flight confirmed it on the box 2026-08-05**, `docs/deploy/nas.md:609`), and ~~⚠ an **external prerequisite in the homelab repo: the drafted tailnet ACL, applied (D2)**~~ — ⏸ **DEFERRED by the user 2026-08-03, paired with a bind-to-LAN design decision this row must honour: see *"Two user decisions, 2026-08-03"* below.** No external prerequisite remains. **All dependencies satisfied; the row shipped** |
| **Touches** | `docs/deploy/nas.md` § *Executed* only. The homelab-side artifacts land in **that** repo |
| **Merge gate** | ✅ **RELEASED — [PR #66](https://github.com/mmackelprang/chat-gateway/pull/66) merged 2026-08-05 as `4ddd6f5`.** Was: ⏸ **YES — deploy + secret-handling path** |
...
⚠ **Corrected 2026-08-05 (CG-79): the premise of that struck sentence is now
false — the deploy happened, and the decision above is what it BUILT.** The
gateway is published on the LAN address only; `docs/deploy/nas.md` §10 records
the bind as executed and demonstrated. **The conclusion the sentence was
defending survives and gets stronger, which is why the sentence had to go:** it
argued *"nothing is leaking, because nothing is running."* What is true now is
*"something is running, and it is bound where the decision said to bind it."*
Those are different guarantees with the same headline, and leaving the weaker
one standing would let a reader think the exposure question was still
hypothetical. The residual is unchanged and stated below: `/healthz` is
unauthenticated and reachable by **anyone on the home LAN**.
...
✅ **Both were carried out on 2026-08-05 and this row has shipped** (noted
2026-08-05, CG-79). C0(b) **PASSED** — `{aiteam-harness: False, job-hunter: True,
aitrader: False}` from the real `load_registry` against the gitignored file
(`docs/deploy/nas.md:609`). The imperative *"Run it"* is left in place rather
than struck: it is an instruction to a Builder, and this row is not the last
thing that will ever touch that box.
...
before it is used.** Passwordless root on a box running **10 live stacks** at the
time this was written — **11 now that the gateway is one of them** (CG-79,
2026-08-05) — including claude-mem's Postgres, is a large blast radius, and `claude` is **not**
...
- 🛑 **`capture.sh`** — it rewrites `nas/compose/*.json` for **every** stack on
  the box (ten when this was written, **eleven** since the gateway joined them —
  which is why the rule is now stated against "every" rather than a number that
  this very runbook increments; CG-79).
...
✅ **This gate is now SATISFIED — the user ran it 2026-08-05 and the output was
read the way this paragraph demands** (recorded 2026-08-05, CG-79). Result:
**zero** hits for `cgw_`, `chat.googleapis.com`, `token=`, `key=`, `private_key`
or a PEM header; every env var present is a path or a non-secret flag. **The
`clean. safe to commit.` line was not what answered it** — the explicit shape
scan was, exactly as written. ⚠ **No tally of that file is reproduced here**, and
the reason is recorded at the top of this row: the first attempt to reproduce one
got it wrong within a single paragraph. The artifact is the **homelab** repo's.
...
host`; any change to an existing NAS app; or reaching for **any**
`chat-gateway-sa*.json` other than `chat-gateway-sa-gw.json`. *(This last named
`iac/chat-gateway-sa.json` specifically until 2026-08-05; that file has since been
**deleted**, so the prohibition is restated against the shape rather than the
path — an old checkout or a backup can still hand a reader a dead key, and the
rule has to survive the file. CG-79.)*
...
### CG-59 · Long-run observation, and what a **deployed** `/healthz` needs  🚀 in flight · ✅ **half 1 SHIPPED 2026-08-05, LIVE ON THE BOX 2026-08-11** · ⚠ **half 2 DIED AT 24 h — see CG-82**
...
> ⚠ **SECOND CORRECTION 2026-08-11 — three things below are now false and one is
> false in an interesting direction.**
>
> 1. ✅ **`?strict=1` IS ON THE BOX.** It rode CG-80's redeploy on 2026-08-11,
>    exactly as the coupling below predicted. Byte-identity re-confirmed there.
>    ⛔ **But `?strict=1` → 503 was NOT observed.** A real degraded window occurred
>    after the restart — the same *"never completed a poll"* reason string this
>    section quotes — and **cleared before it could be sampled.** The 503 path is
>    proven only in the driven local test. This section's table is not evidence
>    from the box and does not become so by the endpoint being deployed.
> 2. ⚠ **THE TILE WAS NEVER REPOINTED BECAUSE IT NEVER EXISTED.** Everything below
>    about *"repointing"* presupposes a live dashboard row. `services.yaml` on the
>    NAS carried **8** tiles and chat-gateway was not among them — so the *"green
>    while inbound is dead"* framing described a **tile that was not probing at
>    all**, which is the same blindness by a shorter route. A tile was **created**
>    2026-08-11, on `?strict=1` from birth. ⚠ **The ORDER this section prescribes
>    survived its own premise being wrong** and was followed.
> 3. ⛔ **THE UPTIME EVIDENCE IS GONE, AND NOT BECAUSE A REBUILD SPENT IT.**
>    `dockerd` died 2026-08-10 and the container did not survive. Last **observed**
>    alive: `2026-08-06T22:29:53Z`, ~25 h witnessed. **CG-82 task 1 is moot** —
>    lost, not spent. This row's strongest single input to the `SubscriberLoop`
>    ⚠ flag is therefore gone, **and a re-run would die the same way** until
>    **CG-85** is decided.
>
> ✅ **The NAS memory half's numbers ARE in, and the verdict is a clean result:
> FLAT — no memory leak, no fd growth, no thread growth, over 25.2 h rather than
> the designed 72.** The **byte-level series** — every `current_bytes`,
> `peak_bytes`, thread and fd figure — lives in `docs/deploy/nas.md` §11 and
> CG-82 and is **not reproduced here**; the verdict and the window are stated
> because they are what this row must be read against. Report it as *flat over
> 25 h*, never as the 72 h run that was designed.
...
>
> ✅ **DISCHARGED 2026-08-11 — and the hazard's own diagnostic became a tool.**
> The rebuild happened, and the *"FastAPI ignores an undeclared parameter"*
> mechanism this paragraph identified is exactly what now **proves which image is
> live**: the old handler answered **200 to any value**, the new one declares
> `strict` as a bool and answers **422 to `?strict=banana`**. One request, no
> degradation of production, no waiting for a fault — you are questioning the
> parser, not the gateway. ⚠ **Two of the three ordering steps completed; the
> middle one did not.** *Verify 503 on a degraded boot* never happened (the window
> closed first), and the third step turned out to be **create**, not repoint —
> there was no tile. ⛔ **And the reason given here for NOT scheduling the redeploy
> was overtaken by events rather than by a decision:** the uptime it was protecting
> was destroyed by the 2026-08-10 `dockerd` outage before any rebuild could spend
> it.
>
...
> — CG-82.** ⚠ **Corrected again 2026-08-11: it did not reach its ceiling either —
> it STOPPED at 25.2 h**, 152 samples, ~90 minutes after the dev-box half, on a
> different host. ✅ **And it answered its question: FLAT — `peak_bytes` unchanged,
> 6 threads and 9 fds constant, no leak.** Numbers have one home each
> (`docs/deploy/nas.md` §11 and CG-82) and are not copied here; the coincidence of
> the two deaths is **CG-85**. As designed it was: ten-minute samples of
...
⛔ **FALSIFIED 2026-08-11 by measuring the box, and the correction is more
interesting than the claim.** The tile existed **in a PR**, not in the config
Homepage serves: `/mnt/datapool/apps/homepage/config/services.yaml` on the NAS
carried **8** tiles in its NAS group and **chat-gateway was not one of them.**
(Whether homelab PR #21 merged is not readable from this repo and is not asserted
either way; what was measured is the live file.) **So for six days this paragraph
described a monitoring surface that did not exist** — and it argued the *milder*
of the two available failures. A tile probing plain `/healthz` reads green while
inbound is dead; **no tile at all reads nothing while the whole gateway is gone**,
which is what happened for 23 hours on 2026-08-10 (**CG-84**). ✅ **A tile was
CREATED 2026-08-11** in that file (backup `.bak-20260811`, NAS group now **9**,
validated by parsing rather than by eye), `siteMonitor` on **`?strict=1`** from
birth, carrying a comment explaining that the `1` is load-bearing because an
invalid value is a **422** and would read DOWN on a healthy gateway. ⚠ **It was
written directly on the box and `capture.sh` has NOT been re-run**, so the homelab
repo does not record it — an outstanding cross-repo handoff this repo cannot
discharge (`nas.md` §9).
...
### CG-80 · The gateway becomes an MCP server — send-only  ✅ **DONE — code 2026-08-10 (PR #72), DEPLOYED AND EXERCISED 2026-08-11** · ✅ **ALL 8 DECISIONS SIGNED OFF 2026-08-10**
...
> ⛔ **SECOND CORRECTION 2026-08-11 — the surviving fragment did not survive, and
> this row is now DONE ON THE BOX.**
>
> - **The uptime evidence was destroyed, not spent.** `dockerd` on the NAS failed
>   **2026-08-10** and the container did not come back with it. Last **observed**
>   alive `2026-08-06T22:29:53Z` — ~25 h witnessed, never the ~5 days four
>   documents asserted. **CG-82 task 1 is MOOT.** ⚠ *Lost, not spent* — and the
>   ordering constraint this row was so careful about turned out to be protecting
>   something that was already gone. **Do not resurrect a capture-first
>   instruction from any paragraph below.**
> - ✅ **DEPLOYED, ENABLED AND EXERCISED 2026-08-11.** Redeploy, an operator key,
>   a registry entry, and both eras driven over real HTTP against the real box,
>   ending in **a real message delivered** and **hard rule #4 refusing
>   `aitrader-alerts` live**. **BOTH UAT gaps named below are CLOSED.** The record
>   has one home: `docs/deploy/nas.md` §10, 2026-08-11 — **not copied here.**
> - ✅ **The plan's own *"single most valuable"* open question is ANSWERED: BOTH
>   eras are load-bearing.** Legacy negotiates from a bare `initialize`; modern
>   refuses to move without three headers and two `_meta` keys. **D4a is
>   vindicated by measurement rather than by argument** — stop calling it open.
> - ⚠ **No ⚠ verification-ledger flag moved, and the spec predicted this exact
>   temptation.** A send through the MCP tool is **the same webhook bytes from a
>   different caller**; `webhook.send` cleared 2026-07-29 and a new ingress
>   neither re-proves nor extends it.

#### ✅ SHIPPED 2026-08-10 — PR #72. ⚠ **And the rebuild did NOT happen.** ✅ **It happened on 2026-08-11 — see the correction above.**

**The code is on `main`; the surface is not on the box, and that was the scope
this Builder was given.** The plan's Task 8 step 6 describes a rebuild +
`app.redeploy`; that step was **deliberately excluded**, because its own ordering
constraint — capture `RestartCount`/`StartedAt` first, **CG-82 task 1** — had not
been discharged and reaching the NAS was not authorized. So this row is ✅ for
*code* and **open for deploy**, which is the same *merged ≠ in effect* split
CG-61 and CG-55 both recorded, arriving for the third time. ⚠ **Two rows now ride
the same next redeploy** — CG-59's `?strict=1` and this one — which is a heavier
handoff than either planned for alone; it is written where the person doing it
will read it (`docs/deploy/nas.md` §9 hazard 2), not only here.

**Suite 390 → 491**, both ends measured with `python3 -m pytest -q` on the day,
not copied. `tests/test_mcp.py` is 101 of them. **No ⚠ verification-ledger flag
was cleared, added, re-priced or reworded** — `git diff main -- src/ | grep -c
"LIVE-UNVERIFIED\|SHAPE-VERIFIED"` → **0**.

⚠ **The plan's code contradicted the plan's own Goal, and the review did not find
it — the build did.** Spec §2's table and the plan's Goal line both promise an MCP
caller inherits **audit journalling**; the plan's `call_tool` took no
`DeliveryLog` and wrote nothing, so an MCP send would have been invisible in
`GET /v1/deliveries` — including in the very row Task 8's own UAT table asks for
as proof a message arrived. Wired through, with `describe_exception` on the
failure detail rather than the `str(exc)[:200]` its `/v1/messages` sibling uses:
a **new** write site gets the current rule, and the old one was deliberately left
alone (spec §15 forbids changing that route). ⚠ **That sibling's `str(exc)[:200]`
was then measured writing a full webhook URL, `key` and `token` included, into the
same delivery log** — pre-existing, out of scope here, and ~~**filed below**~~
⚠ **CORRECTED 2026-08-10: it was NOT filed below.** The only thing filed in this
row's findings was the *`AuthError`/CG-73* item — a different finding. It is now
**CG-83**, filed by the Coordinator on noticing the gap. ⚠ **And its severity is
narrower than "was then measured" reads:** the leaking exception was an **injected
`_LeakyError` in a test**, not production traffic. Every adapter today wraps its
httpx errors into marked classes carrying only `type(exc).__name__`, so nothing
currently reaches that line holding a credential. **Latent, not live** — the full
reasoning, and why it still earns a row, is in CG-83.

**Pre-merge review found sixteen findings across two reviewers; all were fixed.**
The three that were more than tidying:

1. ⚠ **The modern-era header cross-check was bypassable, and the bypass
   DELIVERED.** `_era_of` discriminated on the body alone, so a request carrying
   the modern routing headers but no `params._meta` took the legacy branch where
   `_require_header` never ran — measured: mismatched `Mcp-Method` **and**
   `Mcp-Name`, message delivered, 200. That is the exact hazard
   `_require_header`'s own docstring names. It also meant **§6.3 requirement 4
   was not implemented at all**: a missing `_meta` was indistinguishable from a
   legacy request. Both closed; the bypass now 400s and the message does not send.
2. ⚠ **The blocking send ran on the asyncio event loop.** `/v1/messages` is
   declared `def` so Starlette gives it a worker thread; `/mcp` was `async def`,
   so a 30-second webhook timeout would have stalled **every** concurrent request
   in the process — including the unauthenticated `/healthz`. Hard rule #5's
   endpoint being unanswerable while the gateway looks alive is that rule's own
   subject, arriving through a route declaration. Now dispatched through
   `run_in_threadpool`; measured after the fix, `/healthz` answers in **2 ms**
   with a send in flight.
3. ⚠ **The rule #1 guard — the row's mechanical answer to "does the gateway own a
   second schema" — was blind.** It stripped `enum` and `description` from *every*
   property, so it only really guarded the *set* of property names. Of six
   candidate hand-edits, **six slipped**, including the two the spec names
   explicitly as the leak vector (`description` is a **prompt**) and the one §3
   predicts anyone will want (`cards`). Repaired to narrow `identity` only, then
   re-measured: five caught. **The sixth — a top-level `description` on the
   `inputSchema` itself — needed a third assertion**, because both surviving
   guards are equality comparisons and the thing they must not compare is the
   thing being guarded. Verified by poisoning the real function: the new test goes
   red and **the other three stay green**, which is precisely the blindness it
   covers. ⚠ The queue row and spec both said planning had *"verified the guard
   catches a hand-edit to `cards.items`"* — true, and it was the wrong test to be
   reassured by.

**Two deliberate deviations from the plan's literal text, both flagged rather than
absorbed:** `initialize` now falls back to **`LEGACY_PROTOCOL_VERSION`**, not the
modern one — only a legacy client ever sends `initialize`, and answering it
`2026-07-28` tells exactly the clients this server can serve to disconnect, which
is the D4a failure mode dual-era exists to prevent, produced by the dual-era code
itself. The plan's test pinned the modern value; it was updated and renamed rather
than deleted, with the reason in its docstring. And `params: null` is treated as
**absent** rather than refused, because many serializers emit it for "no params".

⚠ **UAT was thorough but has TWO named gaps, and neither is closed by merging.**
✅ **BOTH WERE CLOSED ON THE BOX, 2026-08-11 — read to the end of this block before
believing the heading**, which is left standing because *"merging does not close
them"* was the correct thing to say and the thing that closed them was a deploy,
not a merge.
Everything below ran against a **real uvicorn process over real HTTP** — both eras,
`cacheScope: private`, cross-tenant enum isolation, the rule-#4 refusal as readable
tool text, all transport guards, the delivery-log row, `/openapi.json` carrying no
leaked enum after two tenants' schema calls, and a regression probe for every
review finding. ~~**Not done: (a) no real MCP client has completed a handshake**, so
*which era is actually load-bearing* — the plan's own "single most valuable thing
this task produces" — **is still unmeasured**; and **(b) no message has reached a
real Google Chat space through the tool**, because that needs the live gitignored
registry and real webhook credentials, and posting into the user's live spaces was
not authorized.~~ The failure path *was* exercised end to end against an unroutable
host. **Neither gap is a ledger question** — spec §11's named temptation is
declined; a round trip through this tool would be the same bytes from a different
caller and clears nothing.

✅ **BOTH GAPS CLOSED 2026-08-11, on the deployed box, and the answer to (a) is
worth more than the closure.** (b) is simple: **a real message was delivered**
through `tools/call` over the **modern** era — `{"status": "delivered", "channel":
"google_chat", "identity": "pm-familyworkspace", "mode": "webhook", "thread_key":
null}` — with an audit row in `GET /v1/deliveries` and, correctly, **no row for the
refused attempt beside it**. (a) is the interesting one: ✅ **which era is
load-bearing? BOTH, and asymmetrically.** Legacy `2025-11-25` is reachable with a
**bare `initialize`** and nothing else. Modern `2026-07-28` will not move without
**all five** of `params._meta`'s two `io.modelcontextprotocol/*` keys and the
`MCP-Protocol-Version` / `Mcp-Method` / `Mcp-Name` headers — each omission its own
distinct 400. **A server that had picked one era would have been silently
unreachable to the other's clients, exactly as D4a said.** ⚠ **So D4a is vindicated
by measurement rather than by argument**, which is a different and stronger
standing than the one it was decided on; the row should say *answered*, not *open*.
⚠ **And `cacheScope: 'private'` was confirmed ON THE WIRE** via `server/discover`
(alongside `resultType: 'complete'`, `ttlMs: 300000`) — the hard-rule-#4 control
this section calls a leak vector, checked where an intermediary would read it
rather than where a test asserts it. ⚠ **None of this moves a ledger flag**, and
the sentence above saying so was written *before* the round trip existed, which is
the only reason it can be trusted now: it declined the clear in advance.

⚠ **One finding filed for the Planner, NOT fixed here — it belongs to CG-73.**
`mcp.py`'s 401 path interpolates `str(exc)` on `AuthError`, which is **not** a
`GatewayAuthoredError`. Safe today — all three of its messages are repo-authored
literals — and it is exact parity with the pre-existing `service.py` 401. What is
missing is the *checkability* `errors.py` exists for: because the class is
unmarked, `tests/test_error_surfaces.py` does not read its raise sites, so a
future `raise AuthError(f"…{value}")` ships unguarded. **This is a fourth site in
CG-73's category** (*drift in a hard-rule-#2 control, not a proven leak*), and
marking `AuthError` would change the existing `/v1` 401 detail — a decision, not a
drive-by. Recorded, not actioned; CG-73 is not this row's to edit. Two smaller
ones for the same reader: `POST /mcp` has **no body-size cap** (parity with
`/v1/messages`; the right home is a repo-wide limit, not `mcp.py`), and
`GET`/`DELETE /mcp` answer **405 unauthenticated**, disclosing the surface exists
— materially moot, since `/healthz` already publishes `mcp.enabled` unauthenticated
on the same LAN by design.
...
| **Depends on** | nothing in code. ~~⚠ **Sequenced after CG-59's soak**~~ ✅ **pulled forward 2026-08-10**; ~~one residual — **CG-82 task 1 captures `RestartCount`/uptime before this row's rebuild.**~~ ⛔ **that residual is VOID as of 2026-08-11 — the uptime was destroyed by the `dockerd` outage before any rebuild could spend it, so CG-82 task 1 is moot.** See *Deploy coupling* below |
...
**An MCP server surface is another ingress to the send path this gateway already
has.** `POST /mcp` → the same `authenticate()`, the same
`registry.identity_for`, the same adapter. Everything below the tool call is
unchanged code, which is the whole architectural claim and is what makes hard
rule #4 hold **by reuse** rather than by a second implementation that can drift.

**The research verdict has ONE home and it is the spec's §1** — no Google Chat
MCP server exists from Google; zero MCP servers subscribe to Chat events; the
three inbound "Chat channel" bridges all ride a Claude-Code-proprietary
extension rather than standard MCP; `mcp.so` returned 403 and **remains an
unsearched gap, recorded rather than papered over**. Do not restate any of it
here or in `CLAUDE.md`.

**Three findings from the planning are worth the row's space, because each one
changed the design:**

1. ⚠ **The protocol broke nine days before this row was written, and picking
   wrong makes the endpoint SILENTLY UNREACHABLE.** Revision `2026-07-28` is not
   a point release — it deletes `initialize`, `notifications/initialized`, `ping`
   and `Mcp-Session-Id`, and adds `server/discover`, per-request `_meta`, three
   required headers and `resultType`/`ttlMs`/`cacheScope`. **Modern client →
   legacy server fails, and legacy client → modern server fails.** Filed as
   decision **D4a**; the recommendation is dual-era on one endpoint, and it
   roughly **doubles the protocol layer**. ⚠ *The spec's §16 sizing was drafted
   before this finding arrived and now says so, rather than leaving the earlier
   estimate standing — CG-69 category (b).*
2. ⚠ **`cacheScope` must be `"private"`, and that is a hard rule #4 control
   rather than a performance knob.** `"public"` asserts a response carries no
   user-specific data and MAY be cached **across authorization contexts** — and
   this server's tool list **does** vary by API key, because `identity`'s enum is
   that app's allowlist. `"public"` would let an intermediary serve one tenant's
   identity allowlist to another: **a rule #4 violation delivered by a cache
   header**, and invisible to a review that is looking at the auth check.
3. ⚠ **A pydantic detail that would have been a silent global breach.**
   `send_message_schema` mutates the dict `model_json_schema()` returns. Had
   pydantic handed back a **cached** dict, one app's identity enum would persist
   into the next caller's schema *and* into `/openapi.json`, which serves the
   same model to every unauthenticated reader of `/docs`. **Measured: it does
   not**, and the plan pins it, because the failure mode is silent and an upgrade
   could introduce it.

**Hard rule #1 gets a mechanical answer, not a reassurance.** A tool
`inputSchema` is a schema and a tool `description` is a **prompt**, so both are
places app-domain knowledge leaks in — invisibly, because it reads as
helpfulness. The discriminator is whether a human typed a property name, so the
schema is **generated** from `OutboundMessage.model_json_schema()` and a test
compares it for equality against a fresh generation. Verified during planning
that the guard **catches** a hand-edit to `cards.items`; a guard that does not is
worth nothing. `cards` therefore stays an opaque array — **a card-builder tool is
the thing rule #1 exists to refuse**, and it is the first thing anyone will want.

**Hard rule #5: one field, no counters, and the verdict is reasoned rather than
reflexive.** `mcp: {enabled, tools}` — never an input to `status`, never a
`reasons` entry at any value. Every counter this repo has guards a loop, thread,
queue or disk write; `/mcp` is synchronous, so a failure is loud in the same
round trip and there is no silent state for a counter to catch. What the field
**is** for is CG-59's lesson one turn later: a deployed container answered `200`
to `?strict=1` because FastAPI ignores an undeclared query parameter, so an
operator could not tell a rebuilt image from a stale one by probing. It discloses
strictly **less** than `registry.health()` already does on the same
unauthenticated response.

⚠ **No ⚠ verification-ledger flag is cleared, added, re-priced or reworded**, and
the spec says so in the ledger's own terms rather than by silence. This sits
above `adapters/` and makes no Google call of its own. **The temptation is named
because a Builder will meet it:** a live round-trip through the MCP tool will
*look* like fresh Google evidence. It is not — the same bytes from a different
caller, and a clear earned that way is the same category error as claiming one
from an offline replay.

#### Deploy coupling — ~~read before scheduling this~~ ✅ **EXECUTED 2026-08-11; kept as the record of what was scheduled and what actually happened**

⚠ **The redeploy this row needs is also the vehicle that finally lands CG-59's
`?strict=1` on the box, and the ORDER is not this row's to invent.**
`docs/deploy/nas.md` §9 hazard 1 sets it: **rebuild + `app.redeploy` → verify
`?strict=1` returns 503 on a degraded boot → only then repoint the Homepage
tile** (a homelab change). The running container predates `?strict=1` and answers
**200** to it today, so repointing first *changes nothing while looking exactly
like the fix*.

✅ **Done 2026-08-11 — two of the three steps as written, and the third turned out
to be a different verb.** The rebuild ran (`app.redeploy`, its **first real
exercise**), `?strict=1` is live, and the dashboard row now
points at it. ⛔ **The middle step did NOT happen:** *verify `?strict=1` returns
503 on a degraded boot*. A genuine degraded window occurred after the restart —
the *"never completed a poll"* reason — and cleared before it could be sampled, so
the 503 path is **still proven only in the driven local test.** ⚠ **And the third
step was CREATE, not repoint:** there was no Homepage tile for this service in the
config the NAS serves. **Both deviations are recorded rather than smoothed**,
because *"the order was followed"* would otherwise read as *"the verification was
done"*, and it was not.
...
⛔ **AND THEN THE UPTIME DIED TOO — corrected 2026-08-11, and this is the third
correction to one paragraph, which is itself the finding.** `dockerd` on the NAS
failed on **2026-08-10** and the container did not survive it. The streak's last
**observed** moment is `2026-08-06T22:29:53Z`, ~25 h witnessed; the *"~5 days …
right now"* above was **arithmetic on a start timestamp**, not an observation, and
the four days it counted are exactly the four days nothing was watching. **CG-82
task 1 is MOOT — it cannot be discharged.** ⚠ **LOST, not SPENT**, and the two are
not interchangeable: this paragraph's entire purpose was to price a deliberate
trade, and no trade occurred. ⚠ **Do not read *"the rebuild happened on 2026-08-11"*
as the capture being behind us** — it was never taken, by anyone, and there is
nothing left to take it from. The careful distinction this paragraph draws between
*sampling* and *uptime* was correct and is kept; what it could not anticipate is
that a third thing — **the host's container runtime** — would end both.
...
running — spec §4 tabulates what a second container would cost on a box with no
registry, and it is disqualifying), and **no new secret**: `GATEWAY_ENABLE_MCP`
holds no credential, so there is no new `.env` key, no new stdin transport and
**no new `SECRETS.template.md` row** — nas.md §6's three-row set is unchanged.
Bonus: it is the **first real exercise of `app.redeploy`**, which nas.md §10
deviation 6 records as documented-but-untested. ✅ **Claimed 2026-08-11: it ran,
against a locally built image on a box with no registry, and it works** — that
deviation no longer says *untested*. *(Job id and image digest: nas.md §10's
2026-08-11 entry, their one home.)*

#### ⚠ Merging is not finishing — the operator action this PR cannot perform ✅ **PERFORMED 2026-08-11**

**The surface ships with no caller.** An agent tenant needs a minted key, an app
entry in the **gitignored** `config/registry.yaml`, and a
`CHAT_GATEWAY_API_KEY__AGENT_MCP` line in the box `.env`. **A PR cannot touch any
of those.** `registry.example.yaml` gains the template and nothing more. This is
**CG-61's lesson restated** — *merged* and *in effect* are two different facts in
this repo and always will be — and the row is not ✅ until an operator has acted
or the user has said the template is enough.

✅ **The operator acted on 2026-08-11 and the row is now ✅.** The key was minted
(32 random bytes, hex) and appended to the box `.env` **over stdin, never argv**
(rule #2), mode `600 root:root` preserved, four `CHAT_GATEWAY_API_KEY__*` names now
present; `agent-mcp` was written into the gitignored registry (backup
`.bak-20260811`, re-parsed and asserted afterwards rather than assumed); and
`GATEWAY_ENABLE_MCP` was set in the compose via `app.update`.
**Env-var NAMES only in this repo — no value of any kind is recorded anywhere in
it.** ⚠ **The paragraph above is kept in the present tense it was written in**,
because *merged ≠ in effect* was right for a day and is the third time this repo
has recorded that split (CG-61, CG-55, this row).

⚠ **ONE GRANT WAS WITHHELD DELIBERATELY, AND IT IS NOT A DEFAULT.** `agent-mcp`
carries **one** identity, `pm-familyworkspace`. **`aitrader-alerts` and
`aitrader-reports` were NOT granted** — and because **D7 declined a registry
`allow_mcp` flag**, that identity list is the **only** control that exists over
what an MCP caller may send as. The consequence flagged at the foot of this
section — *"one surface, no distinction means every registered identity becomes
model-addressable to whoever holds the key"* — is therefore **answered in this
deployment by the mechanism that already existed**, which is what D7 said the user
could do instead of a new flag. ⚠ **Recorded so that adding an identity to this app
later is understood for what it is:** widening a real-money tenant's
model-addressable surface, not editing a config line. Hard rule #6 and
`docs/consumers/aitrader.md` §8 are untouched either way — this is strictly
outbound.
...
explicitly **delegated to the Planner** — so D1 and D2 below are answers, not
re-asks.

| # | Question | Planner's recommendation |
|---|---|---|
| D1 | inbound scope | **send-only** — `read_inbox` designed in full and filed as **CG-81**, not queued |
| D2 | process model | **(a)** mounted route on the existing FastAPI app |
| D3 | how many tools | **one**, `send_message` |
| D4a | which protocol era | **dual** |
| D4 | hand-rolled or the `mcp` SDK | **hand-rolled**, no new dependency — ⚠ D4a makes this closer than it looked |
| D5 | feature flag | **`GATEWAY_ENABLE_MCP`, default OFF** |
| D6 | `/healthz` | **`mcp: {enabled, tools}`, no counters, never affects `status`** |
| D7 | a registry `allow_mcp: false` opt-out | **no** — ⚠ but this is the knob if the user wants one for `aitrader`; see below |
| D8 | agent tenant onboarding | **own app id + identity, as an operator action** |

⚠ **One consequence flagged rather than reopened, because it follows from a
settled decision and a real-money tenant is on the other end of it.** *One
surface, no distinction* means **every registered identity becomes
model-addressable to whoever holds the key** — including `aitrader-alerts` and
`aitrader-reports`. That is strictly **outbound**; hard rule #6 and
`docs/consumers/aitrader.md` §8's four-row guarantee are **untouched**. It is
still a new fact about that deployment, and **D7 is the knob** if the user wants
belt-and-braces. Under D2(a) such a flag *is* enforceable, because MCP is a
distinguishable route.

---

### CG-81 · An MCP `read_inbox` tool  🚫 **FILED, NOT QUEUED** · **needs user sign-off naming hard rule #6**

| | |
|---|---|
| **Origin** | designed in full by CG-80's Planner, 2026-08-06, and deliberately **withheld from the queue** |
| **Depends on** | **CG-56** (hard), and **CG-80**, and the user's explicit sign-off |
| **Touches** | `mcp.py`, `docs/consumers/aitrader.md` §8 (**a fifth row, in the same PR**), `docs/integration-guide.md` |
| **Merge gate** | **yes** |
| **Design** | [spec §7](superpowers/specs/2026-08-06-mcp-server-surface-design.md) — the rule #6 analysis, argued **both ways** |

**Do not fold this into CG-80.** Folding a good idea into an open row is the
scope creep this queue keeps correcting, and here it would also ship a **data
loss**.

**Why it is not queued — one reason is dispositive on its own and is not a
policy question.** `Inbox.poll` **drains**: it clears the app's queue, closes the
journal ids and compacts. At-most-once by design; **CG-56 is the row that would
change that and CG-56 has not shipped.** So an MCP `read_inbox` and a tenant's
production poller would be **competing destructive consumers of one queue** — a
model calling it out of curiosity would *silently delete* replies jobhunt's
poller was about to collect, and jobhunt could not detect it, because the audit
trail records what **arrived**, never what left.

**Two further holes, of two different kinds, and neither is settled here.** The
*consumer* of the data changes even though the *authorization* does not — the
same bytes land in a model's context window and, from there, a transcript — and
whether a third door changes rule #6's own **enumeration** (*"Two opt-in paths
exist…"*) is genuinely arguable. ⚠ **It is arguable in a way that is coupled to
CG-80's D2:** under a stdio shim, `read_inbox` would unambiguously *not* be a
third path; under the mounted route CG-80 recommends, the gateway grows a new
authenticated route that reads the inbox and **the enumeration argument bites
harder**. Choosing D2(a) makes this question sharper, not softer. Both sides are
in spec §7.3–7.4 — **do not summarize them here, and do not decide them in a
Builder cycle.**

⚠ **`aitrader` is untouched mechanically and NOT untouched contractually.**
`allow_inbound: false` means the tool never appears in its `tools/list` and is
refused if called anyway. But `docs/consumers/aitrader.md` §8 is a **four-row
enumeration** framed as *"the mechanism exists, and this app is locked out of
every part of it"*, explicitly chosen because it *"survives the gateway growing
more inbound features."* This **is** the gateway growing one. That table gains a
**fifth row in the same PR** — not a follow-up, on CG-75's precedent that rule #5
does not permit leaving a false statement standing for the duration of a second
PR.

**⚠ And one thing this row would NOT fix, because nothing can.** MCP servers
cannot send requests, cannot send unsolicited notifications and cannot cause a
model turn — `2026-07-28` removed server-initiated requests outright and deleted
the GET stream. So `read_inbox` can only ever be **polling a model must remember
to do**, which is strictly worse than the `callback_url` push jobhunt already
uses. That is the honest case against building it at all, and it is recorded here
rather than discovered after sign-off.
...
⚠ **This section is VESTIGIAL and has disagreed with the table since 2026-08-05 —
noted 2026-08-11 rather than fixed, because fixing it means choosing a convention
and that is the user's call, not a Builder's.** It says *"nothing"*; **CG-59 has
been 🚀 in flight since 2026-08-05.** **The table above is the operative record of
status — read it, not this heading.** The same caveat applies to **Blocked** and
**Recently shipped** below, both of which stopped being maintained around
2026-07-31 while the table went on moving. Filed as an observation here rather
than as a row, because it is bookkeeping about this file and CG-69 already owns
that subject.
...
narrows CG-12's own residue claim.** `apps_for_space` (`registry.py:248-259`)
